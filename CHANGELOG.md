...
* Standalone Topic Operator now reads certificates directly from the Kubernets Secrets in PEM format instead of using JKS/P12 keystore and truststore files.
  ...
  * Use the environment variable `STRIMZI_TRUSTSTORE_SECRET_NAME` to configure the CA certificates for TLS encryption when cnnecting to the Apache Kafka cluster.
  * Use the environment variables `STRIMZI_KEYSTORE_SECRET_NAME`, `STRIMZI_KEYSTORE_KEY_NAME`, and `STRIMZI_KEYSTORE_CERTIFICATE_NAME` to configure client certificate the mTLS authemtication when connecting to the Apache Kafka cluster.
