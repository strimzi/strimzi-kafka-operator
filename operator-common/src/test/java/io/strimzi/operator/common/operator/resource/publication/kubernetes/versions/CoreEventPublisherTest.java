...
class CoreEventPublisherTest {
    ...
    void testPopulatesExpectedFields() {
        CoreEventPublisher eventPublisher = new CoreEventPublisher(clock, client, "cluster-operator-id");
        ...
        verify(client.v1().events()).create(eventCaptor.capture());

        Event publishedEvent = eventCaptor.getValue();
        assertThat(publishedEvent.getInvolvedObject().getKind(), is("Pod"));
        assertThat(publishedEvent.getInvolvedObject().getName(), is("example-pod"));
        assertThat(publishedEvent.getInvolvedObject().getNamespace(), is("test-ns"));

        assertThat(publishedEvent.getReportingComponent(), is("strimzi.io/cluster-operator"));
        ...
        assertThat(publishedEvent.getMessage(), is(RestartReason.JBOD_VOLUMES_CHANGED.getDefaultNote()));
