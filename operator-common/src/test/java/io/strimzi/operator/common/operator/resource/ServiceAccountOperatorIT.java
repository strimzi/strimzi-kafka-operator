...
public class ServiceAccountOperatorIT extends AbstractResourceOperatorIT<KubernetesClient, ServiceAccount, ServiceAccountList, Resource<ServiceAccount>> {
    ...
    public void testCreateModifyDelete(VertxTestContext context)    {
        ...
        op.reconcile(new Reconciliation("test", "kind", "namespace", "name"), namespace, resourceName, newResource)
                .compose(rr -> {
                    // Wait for the service account to be created and secrets added
                    return Util.waitFor(new Reconciliation("test", "kind", "namespace", "name"), vertx, "token secrets created for service account " + resourceName, "has tokens", 1000,
                ...
                .compose(rr -> op.reconcile(new Reconciliation("test", "kind", "namespace", "name"), namespace, resourceName, modResource))
                ...
                .compose(rr -> op.reconcile(new Reconciliation("test", "kind", "namespace", "name"), namespace, resourceName, null))
                .onComplete(context.succeeding(rrDeleted -> {
                    // it seems the resource is cached for some time so we need wait for it to be null
                    context.verify(() -> Util.waitFor(new Reconciliation("test", "kind", "namespace", "name"), vertx, "resource deletion " + resourceName, "deleted", 1000,
