...
IMPORTANT: If you annotate a `StrimziPodSet` while the Cluster Operator is reconciling it, the annotation can be overwritten without triggering the requested rolling update.
To avoid this race, xref:proc-manual-rolling-update-pods-{context}[annotate the individual pods directly].
