...
.. KRaft only: Recover the the `clusterId` from logs or copies of the Kafka custom resource. Otherwise, you can get it from one of the volumes by spinning up a temporary pod:
+
[source,shell]
----
