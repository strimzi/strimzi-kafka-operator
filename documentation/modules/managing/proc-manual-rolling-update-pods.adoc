...
Annotate each pod that you want to roll.
When multiple pods are annotated, consecutive rolling updates are performed within the same reconciliation run.

NOTE: If a rolling update is already replacing a pod, annotating that pod does not guarantee an additional restart.
