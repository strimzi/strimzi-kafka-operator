...
Log directories are mounted to `/var/lib/kafka/data/kafka-log__BROKER-POD-INDEX__` based on the `spec.kafka.storage` configuration in the `Kafka` custom resource.
