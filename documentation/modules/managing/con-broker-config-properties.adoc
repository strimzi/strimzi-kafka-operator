...
For data durability, you should also set `min.insync.replicas` in your _topic_ configuration and message delivery acknowledgments using `acks=all` in your _producer_ configuration.
...
* `num.network.threads` defines the number of network threads for the Kafka cluster.
* `queued.max.requests` defines the number of requests allowed in the request queue.
* `num.io.threads` defines the number of I/O threads for a Kafka broker.
* `num.recovery.threads.per.data.dir` defines the number of threads used for log loading at startup and flushing at shutdown. 
Try setting this to at least the number of CPU cores.
...
== Setting up retention limits for a delete policy
...
An out-of-sync follower is usually not eligible as a leader should the current leader fail, unclean leader election is allowed.
...
* xref:reliability_guarantees[Reliability guarantees using transactions]
* xref:data_durability[Data durability]
* xref:con-broker-config-properties-unclean-{context}[Unclean leader election]