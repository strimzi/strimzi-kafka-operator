...
When you enable log compaction for a topic by setting `cleanup.policy=compact`, Kafka uses the log cleaner as a background process to perform the compaction.
...
After compaction, only the tombstone marker is retained. 
The marker is retained for a duration specified by `log.cleaner.delete.retention.ms` to allow consumers enough time to recognize the deletion.
