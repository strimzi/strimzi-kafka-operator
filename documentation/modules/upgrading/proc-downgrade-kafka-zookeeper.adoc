...
When downgrading a ZooKeeper-based Strimzi Kafka cluster to a lower version, like moving from {KafkaVersionHigher} to {KafkaVersionLower}, ensure that the `inter.broker.protocol.version` property in the Kafka configuration aligns with a version supported by the specified Kafka version (`spec.kafka.version`). 
