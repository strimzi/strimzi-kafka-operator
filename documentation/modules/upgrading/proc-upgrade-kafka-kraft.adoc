...
[role="_abstract"]
Upgrade a KRaft-based Strimzi Kafka cluster to the latest supported Kafka version and KRaft metadata version.
...
. If configured, check that the `spec.kafka.metadataVersion` is set to the _current_ version.
+
...
+
WARNING: You cannot downgrade Kafka if the `metadataVersion` for the new Kafka version is higher than the Kafka version you wish to downgrade to. 
