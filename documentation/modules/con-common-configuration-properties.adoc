...
[id='con-common-deployment-strategy-{context}']
=== Deployment strategy type

Use `template.deployment.deploymentStrategy` to specify the strategy used to replace old Pods by new ones on configuration changes.

The value can be:

- RollingUpdate (default): Pods are restarted with zero downtime.
