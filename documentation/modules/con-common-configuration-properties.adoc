...
By reserving enough `memory` and `cpu` resources you can ensure a stable performance of Kafka.
...
For Kafka, the following aspects of a deployment can impact the use of resources:
...
image:common-config-resources.png[Boundaries of a resource request and limit]

If you don't set a limit, the container is given a default limit.
