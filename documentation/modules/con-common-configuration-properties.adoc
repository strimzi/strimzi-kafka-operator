...
In the following situations, in-place resource updates are not possible. 
Strimzi uses a rolling update instead:
...
..Example `KafkaConnect` resource with in-place resizing and deferred waiting
[source,yaml,subs="attributes+"]
----
...
Similarly, when you decrease the memory resources below the maximal Java heap size, you might out-of-memory errors are possible.

To learn more about in-place pod resizing, see the Kubernetes documentation on link:https://kubernetes.io/docs/tasks/configure-pod-container/resize-container-resources/[resizing CPU and memory resources assigned to containers^].
