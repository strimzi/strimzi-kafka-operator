...
You can override the default image by specifying the image and version in the custom resource.
...
NOTE: When configuring the custom resource, to avoid version-image mismatches, specify the `version` property and let the Cluster Operator automatically select the correct image from its mappings to ensure compatibility.
If you need to change the default images used for different Kafka versions, configure the Cluster Operator's environment variables.
