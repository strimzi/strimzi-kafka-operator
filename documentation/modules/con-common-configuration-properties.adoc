...
When using custom images, upgrades must keep the `image` and `version` in sync.
The Cluster Operator cannot update custom images automatically during an upgrade.
