...
The following Strimzi components run inside a Virtual Machine (VM):
...
For Kafka Connect, avoiding over allocation may be the most important concern, especially in distributed mode where the effects of over-allocation are multiplied by the number of consumers.
