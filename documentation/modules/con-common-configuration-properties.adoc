...
Strimzi supports Prometheus metrics using Strimzi Metrics Reporter to directly expose the metrics provided by Apache Kafka (JMX is not required).
...
NOTE: Using `strimziMetricsReporter` is only supported in the Kafka component.
