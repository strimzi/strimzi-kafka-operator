...
[role="_abstract"]
Use the `KafkaUser` custom resource to configure an authorization mechanism for clients that require access to a Kafka cluster. 
Configure user authorization using the `authorization` property in `KafkaUser.spec`.
Enable an authorization mechanism using the `type` field.
