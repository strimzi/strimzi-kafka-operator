...
* Use the current `user.p12` and `user.password`, or `user.crt` and `user.key`, published in their _<user-name>_ Secret to connect to the cluster, when using the User Operator to provision client certificates.
For workloads running inside the same Kubernetes cluster, Secrets can be mounted as a volume so the client Pods construct their key- and truststores from the current state of the `Secrets`.
...
Failure to do this by the end of the renewal period could result in client applications being unable to connect.
