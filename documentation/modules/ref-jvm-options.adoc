...
[CAUTION]
====
The following option will create one or more file(s) in your running containers. This should only be enabled when Garbage Collection logs are required in a file for debugging.
====
...
Enabling this option will write log files to the `/var/<component_name>/diagnostics` folder inside the container.
Log rotation will be applied to the files with a maximum number of 5 files and a maximum file size of 40M.
To enable it, set both the `gcLoggingEnabled` and `gcLogToFile` properties as follows:
