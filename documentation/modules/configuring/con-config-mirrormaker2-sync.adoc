...
By default, the `offset-syncs` topic used by the `MirrorCheckpointConnector` is created in the source cluster.
You can change this behavior by setting the `offset-syncs.topic.location` connector configuration to `target`.

Placing the `offset-syncs` topic on the target cluster allows the `MirrorCheckpointConnector` to run when it has read-only access to the source cluster.
