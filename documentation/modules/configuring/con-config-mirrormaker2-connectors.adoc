...
[role="_abstract"]
MirrorMaker 2 uses a set of connectors to synchronize data between Kafka clusters. 
...
Because it produces heartbeat records to the source Kafka cluster, the `MirrorHeartbeatConnector` typically runs in a Kafka Connect cluster that has write access to the source cluster.
