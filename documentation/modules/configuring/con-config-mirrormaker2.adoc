...
* `target`: Configuration of the Kafka cluster used as the target cluster. Kafka Connect uses this cluster to store internal topics.
* `mirrors`: Defines replication between the source and target clusters.
* `source.alias`: Unique identifier for the source Kafka cluster.
* `source.bootstrapServers`: Bootstrap address of the source cluster. Use the format `<cluster_name>-kafka-bootstrap:<port>`.
