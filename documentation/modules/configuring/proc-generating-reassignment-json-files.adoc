...
NOTE: If you use `spec.topicName` to name your topics, specify that name in the list.
Otherwise, the `kafka-reassign-partitions.sh` tool will not return the topic.
If you don't use `spec.topicName`, specify the `metadata.name` for the topic.
