...
The name must be unique within the Kafka cluster.
...
* `configuration.useServiceDnsDomain`: Uses fully qualified DNS names for bootstrap addresses.
...
* `spec.kafka.listeners[].configuration.useServiceDnsDomain: true` uses fully qualified DNS names that include the cluster service suffix.

== Internal listener with TLS and mTLS authentication
