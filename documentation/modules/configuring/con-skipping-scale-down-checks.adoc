...
Strimzi runs the same kind of check before it removes the broker role from a node and before it removes a JBOD volume from a broker, and one annotation turns off all three checks.
...
This annotation instructs Strimzi to skip the scale-down check, the broker role removal check, and the JBOD volume removal check.
Replace `my-kafka-cluster` with the name of your specific `Kafka` resource.

To restore the checks, remove the annotation:
