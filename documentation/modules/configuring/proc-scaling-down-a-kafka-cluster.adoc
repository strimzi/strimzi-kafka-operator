...
* You have generated a reassignment JSON file named `_reassignment.json_`.
...
. Once all the partition reassignments have finished, the brokers being removed should not have responsibility for any of the partitions in the cluster.
You can verify this by checking that the broker's data log directory does not contain any live partition logs.
If the log directory on the broker contains a directory that does not match the extended regular expression `[a-zA-Z0-9.-]+\.[a-z0-9]+-delete$`, the broker still has live partitions and it should not be stopped.
+
