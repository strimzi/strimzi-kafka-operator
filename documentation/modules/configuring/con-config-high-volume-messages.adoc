...
But even a data source with moderate throughput can produce a high volume of messages if it has a lot of existing data. 
...
Producer and consumer configuration helps to control the size and frequency of requests to Kafka brokers. 
...
You can also use the same configuration options with the Kafka Connect runtime producers and consumers used by Kafka Connect and MirrorMaker 2.0 source connectors.
Consumers are used by source connectors to retrieve data from an external data system for Kafka Connect or from a source Kafka cluster for MirrorMaker 2.0.
Producers are used by source connectors to send messages to a target Kafka cluster.
...
Configure the `fetch.max.bytes`, `max.partition.fetch.bytes`, and `max.poll.records` values at the source connector level (`consumer.*`), as they relate to the specific consumer that fetches messages from the source.
...
For Kafka Connect, the data replication pipeline to the target Kafka cluster is as follows:

.Data replication pipeline for Kafka Connect
*external data source -> (Kafka Connect tasks) source message queue -> producer buffer -> target topic* 

For MirrorMaker 2.0, the data replication pipeline to the target Kafka cluster is as follows:

.Data replication pipeline for MirrorMaker 2.0
...
Consumer and producer configuration is added for the source connector, which is managed using the `KafkaConnector` custom resource. 

.Example KafkaConnector configuration for handling high volumes of messages
[source,yaml,subs="+quotes,attributes"]
----
...
.Example curl request to add connector configuration for handling high volumes of messages
