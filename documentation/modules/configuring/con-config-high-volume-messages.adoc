...
You can also use the same configuration options with the producers and consumers used by Kafka Connect runtime and MirrorMaker 2.0 source connectors and sink connectors.

Source connectors:: 
* Consumers retrieve messages from a source Kafka cluster for MirrorMaker 2.0. As Kafka Connect fetches data from an external data system, consumers are not usually required for source connectors.  
* Producers send messages to a target Kafka cluster for Kafka Connect and MirrorMaker 2.0.

Sink connectors:: 
* Consumers retrieve data from a source Kafka cluster for Kafka Connect.
* Producers push data to an external data system for Kafka Connect.
