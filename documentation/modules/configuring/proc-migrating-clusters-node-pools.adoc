...
----
+
After restarting, the Cluster Operator will log the following warning message:
"KafkaNodePool kafka in namespace _<namespace>_ was ADDED, but the Kafka cluster my-cluster to which it belongs does not have KafkaNodePool support enabled".
It means that, while the node pool for the Kafka brokers was created, it is not handled by the Cluster Operator yet.
