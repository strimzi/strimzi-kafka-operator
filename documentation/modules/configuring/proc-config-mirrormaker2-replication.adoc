...
<42> RACK OPTION: xref:type-Rack-reference[Rack awareness] configuration for the deployment. Use this option only if you need to spread replicas across different racks, data centers, or availability zones. The `topologyKey` must match a node label containing the rack ID. This configuration specifies a zone using the standard `topology.kubernetes.io/zone` label.
