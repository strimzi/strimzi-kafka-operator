...
== Configuring which volume will be used to store the KRaft metadata log

When running a Kafka cluster in a KRaft mode, each node has to store the Kraft metadata log on one of its data volumes.
By default, the KRaft metadata log will be stored on the volume with the lowest ID.
The option `kraftMetadata` can be used to indicate that another volume should be used to store the metadata log.

.Example JBOD storage configuration that will use the volume with ID 1 to store the Kraft metadata
[source,yaml,subs="attributes+"]
----
apiVersion: {KafkaApiVersion}
kind: KafkaNodePool
