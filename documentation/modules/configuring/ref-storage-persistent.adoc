...
[role="_abstract"]
Persistent data storage retains data even when a pod that uses the storage restarts.
Persistent data storage uses a storage framework called persistent volumes (PV). 
PVs are storage resources, independent of the pods that use them.
Pod configuration uses {K8sPersistentVolumeClaims} (PVCs) to make storage requests on the PVs.
...
Persistent storage options might include SAN storage types or a {K8sLocalPersistentVolumes}.
...
.Example persistent storage configuration with 1000Gi `size`
[source,yaml]
----
# ...
storage:
...
== Log directories for persistent storage
