...
<2> These properties configure the Kafka Connect workers in this cluster. However, certain properties are managed directly by Strimzi and cannot be set within this config property. For more information, see the link:{BookURLConfiguring}#type-KafkaMirrorMaker2TargetClusterSpec-reference[`KafkaMirrorMaker2TargetClusterSpec` schema reference^].
