...
Certain properties are managed directly by Strimzi and cannot be set within the `config` property. For more information, see the link:{BookURLConfiguring}#type-KafkaMirrorMaker2TargetClusterSpec-reference[`KafkaMirrorMaker2TargetClusterSpec` schema reference^].
