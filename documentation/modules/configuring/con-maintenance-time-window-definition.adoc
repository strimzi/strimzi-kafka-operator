...
They **do** apply in the following scenarios:

* Automatic renewal of Strimzi-managed internal certificates, such as those used by Kafka nodes.
* Time-based renewal of server certificates signed by a custom Certificate Authority (CA), _if_ Strimzi manages the renewal process based on certificate expiry.

They **do not** apply in the following scenarios:

* User-driven changes to the `Kafka` custom resource, including configuration changes.
* Manual updates to Cluster CA or Clients CA certificatesm, even if they are Strimzi-managed, since these updates are treated as user-initiated actions.
* Externally managed certificates, such as:
  * Custom listener certificates you provide directly
