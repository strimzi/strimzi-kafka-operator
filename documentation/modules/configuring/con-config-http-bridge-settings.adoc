...
= Tuning HTTP Bridge internal settings
...
You can adjust its behavior by setting properties in the `spec.config` section of the `KafkaBridge` resource.

Certain properties are managed directly by Strimzi and cannot be set in `spec.config`.
...
Additionally, the `bridge.id`, `bridge.tracing`, and `bridge.metrics` options cannot be overridden.
...
Configure the executor service behavior using `spec.config`.

The executor service manages the thread pool used to process HTTP requests.
...
If not specified, the default is `max(4, <available CPUs> * 2)`.
...
You can configure additional HTTP Bridge properties as required.
