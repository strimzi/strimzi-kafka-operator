...
The `metadataVersion` or `inter.broker.protocol.version` property for the Kafka `config` must be a version supported by the specified Kafka version (`spec.kafka.version`).
