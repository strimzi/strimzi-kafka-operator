...
You enable the heartbeat connector by including its configuration in the MirrorMaker 2 deployment configuration.
...
If heartbeats are enabled, you can set the `emit.heartbeats.enabled` property of the heartbeat connector to `false` if you do not want to use them.
