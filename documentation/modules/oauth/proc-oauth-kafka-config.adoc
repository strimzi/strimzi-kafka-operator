...
+
For example, here OAuth 2.0 is assigned to the Kafka Bridge client using a client ID and secret, and mTLS:
+
[source,yaml,subs="+quotes,attributes"]
----
apiVersion: {KafkaApiVersion}
...
<3> Trusted certificates for mTLS connection to the authorization server.
+
