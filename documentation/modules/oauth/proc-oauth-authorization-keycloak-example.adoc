...
If the client accesses the Kafka cluster over the Ingress or the OpenShift Route the Kafka bootstrap hostname will be different, and you may need to specify it as an extra argument to the `openssl` command in order to get the certificate containing the proper SNA records:
[source,shell]
----
KAFKA_BOOTSTRAP_EXTERNAL=kafka-bootstrap-addr.somedomain.ext
STOREPASS=storepass
