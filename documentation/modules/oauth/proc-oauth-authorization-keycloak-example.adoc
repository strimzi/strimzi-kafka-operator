...
Kafka and Kafka clients must connect to Keycloak for authorization to work. Connection to Keycloak is indirect. The connection routes through an ingress HTTP gateway using a hostname on port 443. Kafka clients use the same hostname and port as the Kafka brokers. The gateway is part of the Kubernetes infrastructure. This example assumes that Keycloak is deployed using the Keycloak Operator. The operator makes the gateway hostname visible to pods in the same Kubernetes cluster. The hostname must be visible for the example to work. If it's not visible, try deploying Keycloak to a separate Kubernetes cluster. The pods can then access Keycloak using the external hostname to make the connection.
