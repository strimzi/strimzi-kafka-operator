...
+
Usually there is not one single certificate, but a certificate chain. You only have to provide the top-most issuer CA which is listed as last in the `/tmp/sso.pem`.
...
----
+
NOTE: The proper way to obtain the trusted CA certificate is from a trusted source, and not by using the `openssl` command.
...
+
If the client accesses the Kafka cluster over the ingress or OpenShift route, the Kafka bootstrap hostname is different, and you may need to specify it as an extra argument to the `openssl` command in order to get the certificate containing the proper SNA records:
+
[source,shell]
----
KAFKA_BOOTSTRAP_EXTERNAL=kafka-bootstrap-addr.my-domain.ext
STOREPASS=storepass

echo "Q" | openssl s_client -showcerts -servername $KAFKA_BOOTSTRAP_EXTERNAL -connect $KAFKA_HOST_PORT 2>/dev/null | awk ' /BEGIN CERTIFICATE/,/END CERTIFICATE/ { print $0 } ' > /tmp/my-cluster-kafka.pem
----
+
The obtained `.pem` file is usually not one single certificate, but a certificate chain. You only have to provide the top-most issuer CA which is listed as last in the `/tmp/my-cluster-kafka.pem`.
You can extract it manually or using the following command:
+
.Example command to extract the top CA certificate in a certificate chain
[source,shell,subs="+quotes"]
