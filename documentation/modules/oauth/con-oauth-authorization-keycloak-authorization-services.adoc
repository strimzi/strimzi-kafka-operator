...
To run leader selection for topic partitions you need `Alter` permission on the `Cluster:kafka-cluster`.
...
To generate partition reassignment file the `Describe` on the topics involved.
...
To execute the partition reassignment the `Describe`, and `Alter` permissions are required on `Cluster:kafka-cluster`, and `Describe` permissions are required on the topics involved.
...
To verify partition reassignment the `Describe`, and `AlterConfigs` permissions are required on `Cluster:kafka-cluster`,
