...
. <<example-1, Connection to the authorization server with trusted certificates>>
. <<example-2, Direct connection using an introspection endpoint configuration>>
...
//type-KafkaClientAuthenticationOAuth
...
--
[[example-1]]
.Example 1: External TLS connection to the authorization server with trusted certificates
...
[[example-2]]
.Example 2: External connection to the authorization server using an introspection endpoint
[source,yaml,subs="+quotes,attributes"]
----
external:
  type: loadbalancer
