...
Typically, the client id of the OAuth client that represents the broker is `kafka` (the xref:proc-oauth-authorization-keycloak-example_str[example files] provided with Strimzi use `kafka` as the OAuth client id).
