...
The example starts by deploying the {keycloak-project-name} server with pre-configured realms, requiring no additional configuration.
...
While TLS connectivity is used where necessary, this is still just an example, and additional care and review should be done when creating the production configuration.
====
...
First, determine the external route to access the Admin Console.
...
----
NS=sso
oc get route keycloak -n $NS
----

Second, determine the password for the admin user.

For example, if the {keycloak-project-name} Operator was used to install the {keycloak-project-name} server, the password is stored as a secret called `credential-example-keycloak`:

[source]
----
oc get -n $NS secret credential-example-keycloak -o yaml | grep PASSWORD | awk '{print $2}' | base64 -D
----

Now, login to the Admin Console (you have to use https://HOST for OpenShift route) using `admin` as the username, and the revealed password.
...
We assume that the {strimzi-project-name} Cluster Operator has already been installed on the OpenShift cluster.
...
Assuming the {keycloak-project-name} Operator was used to install the {keycloak-project-name} server instance, and that `sso` namespace (project) was used, we can determine the route hostname:
...
OpenShift Route uses a secure connection (https) which means that we need to prepare a certificate truststore to make it possible for Kafka brokers to speak to the {keycloak-project-name} server.

[source]
----
SSO_HOST_PORT=<sso_route_hostname>:443
STOREPASS=storepass

echo "Q" | openssl s_client -showcerts -connect $SSO_HOST_PORT 2>/dev/null | sed -ne '
/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p
/-END CERTIFICATE-/q
' > /tmp/sso.crt
----

Now we need to deploy the certificate to OpenShift as a secret:
...
Set the route as an ENV var before deploying the prepared `Kafka` custom resource for the example:

[source]
----
SSO_HOST=<sso_route_hostname>
EXAMPLES_DIR=examples
cat $EXAMPLES_DIR/kafka/kafka-ephemeral-oauth-single-keycloak-authz.yaml | sed -E 's#\${SSO_HOST}'"#$SSO_HOST#" | oc apply -n $NS -f -
----
...
----
NS=sso
oc run -ti --rm --restart=Never --image={DockerKafkaImageCurrent} kafka-cli -n $NS -- /bin/sh
----

NOTE: If `oc` times out waiting on the image download, subsequent attempts may result in an _AlreadyExists_ error.

You can attach to the existing pod by running:

[source,shell]
----
oc attach -ti kafka-cli
----
...
First, we use the route exposing the {keycloak-project-name} to obtain the certificate.
...
echo "Q" | openssl s_client -showcerts -connect $KAFKA_HOST_PORT 2>/dev/null | sed -ne '
/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p
/-END CERTIFICATE-/q
' > /tmp/my-cluster-kafka.crt
...
The roles assigned to a client, such as the `Dev Team A` realm role assigned to the `team-a-client` service account, are presented in {keycloak-project-name} on the  *Service Account Roles* tab from *Clients*.
...
We can authenticate as `bob` by using `curl` and perform password grant authentication to obtain a refresh token which we can then use to configure the Kafka client.
...
A configuration file for `bob` looks like the following:
...
[source,shell]
----
bin/kafka-topics.sh --bootstrap-server my-cluster-kafka-bootstrap:9093 --command-config /tmp/bob.properties \
  --topic x_messages --create --replication-factor 1 --partitions 1
----


User `bob` can list all the topic, whereas `team-a-client` and `team-b-client` can only list topics they have access to:

[source,shell]
----
bin/kafka-topics.sh --bootstrap-server my-cluster-kafka-bootstrap:9093 --command-config /tmp/bob.properties --list
bin/kafka-topics.sh --bootstrap-server my-cluster-kafka-bootstrap:9093 --command-config /tmp/team-a-client.properties --list
...
User `bob` can also read the cluster configuration:
[source,shell]
----
bin/kafka-configs.sh --bootstrap-server my-cluster-kafka-bootstrap:9093 --command-config /tmp/bob.properties \
  --entity-type brokers --describe --entity-default
----
