...
* Long-lived access token obtained manually
...
Using the `PLAIN` authentication mechanism, credentials exchange can be configured to use the following: 
+
--
* Client ID and secret (using the the OAuth 2.0 _client credentials mechanism_)
* Long-lived access token obtained manually+
...
* Contains a `typ` (type) header claim value of `Bearer` to indicate it is an access token
