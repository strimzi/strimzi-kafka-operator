...
To use OAuth 2.0 authentication in the Kafka cluster, you specify, for example, a mutual `tls` listener configuration for your Kafka cluster custom resource with the authentication method `oauth`:
...
We recommend not using `plain` listeners or `external` listeners without TLS encryption enabled.
