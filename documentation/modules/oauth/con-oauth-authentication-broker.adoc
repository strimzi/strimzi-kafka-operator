...
.Configuring fast local JWT token validation for using Kubernetes service accounts

In order to configure the listener for use with Kubernetes service accounts the Kubernetes API server has to be used as an authorization server.
...
- name: external3
  ...
  authentication:
    type: oauth # <1>
    validIssuerUri: https://kubernetes.default.svc.cluster.local # <2>
    jwksEndpointUri: https://kubernetes.default.svc.cluster.local/openid/v1/jwks # <3>
    serverBearerTokenLocation: /var/run/secrets/kubernetes.io/serviceaccount/token # <4>
    checkAccessTokenType: false # <5>
    includeAcceptHeader: false # <6>
...
----
<1> Listener type set to `oauth`.
<2> URI of the token issuer used for authentication. FQDN has to be used - including the `.cluster.local` extension, which may be different if configured differently by Kubernetes cluster administrator.
<3> URI of the JWKS certificate endpoint used for local JWT validation. FQDN has to be used - including the `.cluster.local` extension, which may be different if configured differently by Kubernetes cluster administrator.
<4> The location of the access token for Kafka broker to use to authenticate to Kubernetes API server in order to access the `jwksEndpointUri`.
<5> Turn off the checking of access token type as the claim for that is not present in service account tokens.
<6> Don't send `Accept` header when performing HTTP requests to the JWKS endpoint as Kubernetes API server doesn't like it.
...
<9> (Optional) Additional constraints that JWT token has to fulfill in order to be accepted, expressed as JsonPath filter query. In this example the service account has to belong to `example` namespace in order to be allowed to authenticate.

The above configuration will use `sub` claim from the service account JWT token as the user id. For example, the default service account for pods deployed in `example` namespace will have the username: `system:serviceaccount:example:default`.
...
.Example additional (optional) configuration settings
[source,yaml,subs="+quotes,attributes"]
----
  # ...
  authentication:
...
<3> The prefix used when constructing the user id. Only takes effect if `userNameClaim` is configured.
...
<6> The location of the access token for Kafka broker to use to authenticate to Kubernetes API server in order to access the protected endpoints. Authorization server has to support `Bearer` authentication. That's an alternative to specifying `clientId` and `clientSecret` which uses `Basic` authentication.
