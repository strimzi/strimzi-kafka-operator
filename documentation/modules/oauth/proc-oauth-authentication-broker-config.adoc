...
<9> Additional configuration for the PLAIN mechanism. Clients can authenticate by passing a client ID and secret as a `username` and `password`. Default is `false`. If not specified, clients can authenticate over PLAIN only by passing an access token as the `password` using an `$accessToken:` prefix.
