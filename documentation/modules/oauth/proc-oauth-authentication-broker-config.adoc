...
<7> Set this to `false` in order to disable the OAUTHBEARER mechanism on the listener. At least one of SASL_PLAIN or OAUTHBEARER has to be enabled. Default is `true`.
