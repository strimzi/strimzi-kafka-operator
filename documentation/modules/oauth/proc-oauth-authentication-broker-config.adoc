...
<4> The token claim (or key) that contains the actual username used to identify the user. Its value depends on the authorization server. If necessary, a JsonPath expression like `['user.info'].['user.id']` can be used to retrieve the username from a nested JSON structure. 
