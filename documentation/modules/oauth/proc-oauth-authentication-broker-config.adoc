...
<15> The maximum number of retries to perform if the request to the authorization server fails in unexpected way. The default value is 0 (no retries). When using this option consider reducing the timeout times for `connectTimeoutSeconds` and `readTimeoutSeconds`. Note that retries prevent the current worker thread from being available to other requests. Many of them stalling could make the Kafka broker unresponsive.
<16> The time to pause before attempting another retry. By default this time is zero (no pause) as many issues are per-request network glitches or proxy glitches, but if your authorization server is under stress you may want to set this to 100 ms or more.
<17> A JsonPath query used to extract groups information from JWT token or introspection endpoint response. Not set by default. This can be used by a custom authorizer to make authorization decisions based on user groups.
<18> A delimiter used to parse groups information when returned as a single delimited string. The default value is ',' (comma).
