...
We advise use of OAuth 2.0 over an encrypted interface through configuration of mTLS listeners.
Plain listeners are not recommended.

If the authorization server is using certificates signed by the trusted CA and matching the OAuth 2.0 server hostname, mTLS connection works using the default settings.
...
<6> (Optional) Trusted certificates for mTLS connection to the authorization server.
