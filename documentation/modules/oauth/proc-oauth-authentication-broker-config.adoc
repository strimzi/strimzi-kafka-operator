...
<11> An `audience` parameter passed to the token endpoint when obtaining an access token for inter-broker authentiction or in the name of the client during `OAuth over PLAIN` with `clientId` and `secret`. This only affects the ability to obtain the token, and the content of the token, depending on the authorization server. It does not affect token validation rules by the listener.
<12> A `scope` parameter passed to the token endpoint when obtaining an access token for inter-broker authentication or in the name of the client during `OAuth over PLAIN` with `clientId` and `secret`. This only affects the ability to obtain the token, and the content of the token, depending on the authorization server. It does not affect token validation rules by the listener.
