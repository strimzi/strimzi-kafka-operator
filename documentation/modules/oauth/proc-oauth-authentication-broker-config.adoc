...
----
<1> Some authorization servers may not provide `iss` claim making it impossible to perform an issuer check. In that case don't specify `validIssuerUri` and set `checkIssuer` to `false`. Default is `true`.
