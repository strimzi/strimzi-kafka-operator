...
<2> If your authorization server provides `aud` claim, and you want to enforce audience check, set this to `true`. As a result, the Kafka broker will reject tokens that do not have its `clientId` in their `aud`. Default is `false`.
