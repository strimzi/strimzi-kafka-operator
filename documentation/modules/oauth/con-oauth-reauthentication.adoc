...
The _SASL OAUTHBEARER_ authentication mechanism supports a Kafka feature called the _session re-authentication_ mechanism. 
Session re-authentication is not supported if _SASL_PLAIN_ authentication is used.
