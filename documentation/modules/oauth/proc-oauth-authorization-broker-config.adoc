...
NOTE: xref:ref-kafka-authorization-super-user-{context}[Super users] always have unconstrained access to a Kafka broker regardless of the authorization implemented on the Kafka broker.
...
<6> (Optional) Designated xref:ref-kafka-authorization-super-user-{context}[super users].
