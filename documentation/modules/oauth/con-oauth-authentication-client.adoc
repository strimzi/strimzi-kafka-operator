...
sasl.login.callback.handler.class=io.strimzi.kafka.oauth.client.JaasClientOauthLoginCallbackHandler
----
<1> Client assertion (private key JWT) token file containing the client assertion. Alternatively, use `oauth.client.assertion` configuration option to specify the client assertion value in clear text.
