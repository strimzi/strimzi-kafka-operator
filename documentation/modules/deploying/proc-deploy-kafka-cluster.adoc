...
+
A persistent Kafka cluster deployment can use the corresponding type of volume without any changes to the `Kafka` configuration:

* Amazon EBS volumes on Amazon AWS
* Azure Disk Storage on Microsoft Azure
* Persistent Disk on Google Cloud
* Local persistent volumes on bare metal
