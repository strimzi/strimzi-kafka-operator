...
# ....
env:
  - name: STRIMZI_TLS_TRUSTED_CERTS_SECRET_NAME # <1>
    value: "kafka-ca"
  - name: STRIMZI_TLS_SECRET_NAME # <2>
    value: "topic-operator-user-tls"
  - name: STRIMZI_TLS_KEY_NAME # <3>
    value: "tls.key"
  - name: STRIMZI_TLS_CERT_NAME # <4>
    value: "tls.crt"
...
<2> The name of the `Secret` with the private key and certificate for mTLS client authentication.
<3> The key under which the mTLS private key is stored in the `Secret`.
<4> The key under which the mTLS client certificate is stored in the `Secret`.
