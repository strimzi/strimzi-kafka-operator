...
env:
  - name: STRIMZI_CLUSTER_CA_CERT_SECRET_NAME <1>
    value: "publicKeysSecret"
  - name: STRIMZI_EO_KEY_SECRET_NAME <2>
    value: "privateKeySecret"
# ...
----
<1> The Certificate Authorities used to sign the Kafka and ZooKeeper server certificates.
<2> The secret that contains the private key for the Entity Operator.
