...
. If you enabled TLS with the `STRIMZI_TLS_ENABLED` environment variable, specify the keystore and truststore (deprecated) or specify names of the secrets containing public and private keys used to authenticate connection to the Kafka cluster.
+
...
# ....
env:
  - name: STRIMZI_EO_CERTIFICATE_CHAIN_DATA <1>
    valueFrom:
      secretKeyRef:
        key: entity-operator.crt
        name: my-cluster-entity-operator-certs
  - name: STRIMZI_EO_KEY_DATA <2>
    valueFrom:
      secretKeyRef:
        key: entity-operator.key
        name: my-cluster-entity-operator-certs
  - name: STRIMZI_CLUSTER_CA_CERT_SECRET_DATA <3>
    value: |
      -----BEGIN CERTIFICATE-----
      base64
      -----END CERTIFICATE-----
...
# ...
----
<1> The private key for the Entity Operator.
<2> The certificate chain for the Entity Operator.
<3> The Certificate Authorities used to sign the Kafka and ZooKeeper server certificates.

.Example deprecated TLS configuration
[source,shell,subs=+quotes]
----
# ....
env:
