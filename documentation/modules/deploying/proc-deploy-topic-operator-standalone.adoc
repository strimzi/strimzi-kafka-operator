...
. If you need a custom `SASL` configuration. For example, to access a cluster in a cloud provider with a custom login module like `AWS_MSK_IAM`. The `STRIMZI_SASL_CUSTOM_CONFIG_JSON` is the mechanism to do that.
The property `STRIMZI_ALTERABLE_TOPIC_CONFIG` defaults to `ALL`. When a subset of properties is alterable, use a comma separated list, as below. In the case that a managed Kafka service doesn't allow any `.spec.config` properties to be set in the `KafkaTopic` a value of `NONE` is equivalent to an empty property.
Otherwise, a comma separated list of the properties that can be altered is used.
+
*NOTE:  Only SASL properties can be set with the `STRIMZI_SASL_CUSTOM_CONFIG_JSON` mechanism.*
+
...
env:
  ...
  - name: STRIMZI_KAFKA_BOOTSTRAP_SERVERS
    value: your-instance-here.kafka-serverless.us-east-1.amazonaws.com:9098
# ...
----
<1> Some managed Kafka services don't allow cluster config lookup, so we disable those checks.
<2> Some managed Kafka services limit what properties can be updated, so we use this to define what values can be altered.
<3> JSON with the SASL properties to be set. Only properties beginning with `sasl.` are allowed.
+
