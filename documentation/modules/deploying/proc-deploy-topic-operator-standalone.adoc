...
. (Optional) Set the `STRIMZI_USE_FINALIZERS` environment variable to `true`:
+
...
----
+
Use this environment variable if you want to use finalizers to control xref:con-deleting-managed-topics-{context}[topic deletion].
