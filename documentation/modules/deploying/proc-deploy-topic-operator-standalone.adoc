...
# ...
----
<1> Some managed Kafka services, such as `AWS MSK`, don't allow cluster config lookup, so we disable those checks.
<2> Some managed Kafka services, such as `AWS MSK`, limit what properties can be updated, so we use this to define what values can be set/updated.
