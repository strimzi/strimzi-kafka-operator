        ...
        - name: strimzi-topic-operator
          ...
          env:
            ...
            - name: STRIMZI_LOG_LEVEL # <5>
              value: INFO
            - name: STRIMZI_JAVA_OPTS # <6>
              value: "-Xmx=512M -Xms=256M"
            - name: STRIMZI_JAVA_SYSTEM_PROPERTIES # <7>
              value: "-Djavax.net.debug=verbose -DpropertyName=value"
            - name: STRIMZI_SASL_ENABLED # <8>
              value: "false"
            - name: STRIMZI_SASL_USERNAME # <9>
              value: "admin"
            - name: STRIMZI_SASL_PASSWORD # <10>
              value: "password"
            - name: STRIMZI_SASL_MECHANISM # <11>
              value: "scram-sha-512"
            - name: STRIMZI_SECURITY_PROTOCOL # <12>
              value: "SSL"
            - name: STRIMZI_USE_FINALIZERS # <13>
              value: "false"
----
...
<6> (Optional) The Java options used by the JVM running the Topic Operator.
<7> (Optional) The debugging (`-D`) options set for the Topic Operator.
<8> (Optional) Enables SASL support for client authentication when connecting to Kafka brokers.
The default is `false`.
<9> (Optional) The SASL username for client authentication.
Mandatory only if SASL is enabled through `STRIMZI_SASL_ENABLED`.
<10> (Optional) The SASL password for client authentication.
Mandatory only if SASL is enabled through `STRIMZI_SASL_ENABLED`.
<11> (Optional) The SASL mechanism for client authentication.
Mandatory only if SASL is enabled through `STRIMZI_SASL_ENABLED`.
You can set the value to `plain`, `scram-sha-256`, or `scram-sha-512`.
<12> (Optional) The security protocol used for communication with Kafka brokers.
The default value is "PLAINTEXT".
You can set the value to `PLAINTEXT`, `SSL`, `SASL_PLAINTEXT`, or `SASL_SSL`.
<13> Set `STRIMZI_USE_FINALIZERS` to `false` if you do not want to use finalizers to control xref:con-deleting-managed-topics-{context}[topic deletion].

. If you want to connect to Kafka brokers that are using certificates from a public certificate authority, set `STRIMZI_SECURITY_PROTOCOL` to `SSL` or `SSL_SASL`. Use this property, for example, if you are using Amazon AWS MSK service.
. To enable TLS encryption and mTLS authentication for connections to the Kafka cluster, specify the trusted CAs and the client certificate using the following environment variables:
+
...
# ....
env:
  - name: STRIMZI_TRUSTSTORE_SECRET_NAME # <1>
    value: "kafka-ca"
  - name: STRIMZI_KEYSTORE_SECRET_NAME # <2>
    value: "topic-operator-user-tls"
  - name: STRIMZI_KEYSTORE_KEY_NAME # <3>
    value: "tls.key"
  - name: STRIMZI_KEYSTORE_CERTIFICATE_NAME # <4>
    value: "tls.crt"
  - name: STRIMZI_CLUSTER_NAMESPACE # <5>
    value: "my-kafka-namespace"
# ...
----
<1> The name of the Secret with the public keys of the Certificate Authorities used to sign the Kafka server certificates as `.crt` entries.
<2> The name of the Secret with the private and public keys for mTLS client authentication.
<3> The key under which the mTLS client authentication private key is stored in the Secret.
<4> The key under which the mTLS client authentication public key is stored in the Secret.
<5> The Kubernetes namespace where the TLS Secrets are located.
...
env:
  ...
  - name: STRIMZI_TRUSTSTORE_SECRET_NAME
    value: kafka-ca
