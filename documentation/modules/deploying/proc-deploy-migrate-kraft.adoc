...
The name of the controller node pool created is `controller`, which is required even if you are already using node pools.
...
But the brokers can have any name if they are already contained in a node pool.
