...
WARNING: Do not configure standalone User Operators that connect to different Kafka clusters to watch the same namespace. 
This can cause the operators to compete for the same `KafkaUser` resources, leading to name collisions and unpredictable behavior.
      ...
      containers:
        - name: strimzi-user-operator
          ...
          env:
            ...
            - name: STRIMZI_CA_TYPE <3>
              value: strimzi
            - name: STRIMZI_CA_CERT_NAME <4>
              value: my-cluster-clients-ca-cert
            - name: STRIMZI_CA_KEY_NAME <5>
              value: my-cluster-clients-ca
            - name: STRIMZI_LABELS <6>
              value: "strimzi.io/cluster=my-cluster"
            - name: STRIMZI_FULL_RECONCILIATION_INTERVAL_MS <7>
              value: "120000"
            - name: STRIMZI_WORK_QUEUE_SIZE <8>
              value: 10000
            - name: STRIMZI_CONTROLLER_THREAD_POOL_SIZE <9>
              value: 10
            - name: STRIMZI_USER_OPERATIONS_THREAD_POOL_SIZE <10>
              value: 4
            - name: STRIMZI_LOG_LEVEL <11>
              value: INFO
            - name: STRIMZI_GC_LOG_ENABLED <12>
              value: "true"
            - name: STRIMZI_CA_VALIDITY <13>
              value: "365"
            - name: STRIMZI_CA_RENEWAL <14>
              value: "30"
            - name: STRIMZI_JAVA_OPTS <15>
              value: "-Xmx=512M -Xms=256M"
            - name: STRIMZI_JAVA_SYSTEM_PROPERTIES <16>
              value: "-Djavax.net.debug=verbose -DpropertyName=value"
            - name: STRIMZI_SECRET_PREFIX <17>
              value: "kafka-"
            - name: STRIMZI_ACLS_ADMIN_API_SUPPORTED <18>
              value: "true"
            - name: STRIMZI_MAINTENANCE_TIME_WINDOWS <19>
              value: '* * 8-10 * * ?;* * 14-15 * * ?'
            - name: STRIMZI_KAFKA_ADMIN_CLIENT_CONFIGURATION <20>
              value: |
                default.api.timeout.ms=120000
                request.timeout.ms=60000
            - name: STRIMZI_IGNORED_USERS_PATTERN <21>
              value: "^ANONYMOUS$"
            - name: STRIMZI_PKCS12_KEYSTORE_GENERATION <22>
              value: "true"
----
...
<3> Type of certificate manager. Set to `cert-manager` to enable cert-manager integration.
The default is `strimzi`.
<4> The Kubernetes `Secret` that contains the public key (`ca.crt`) value of the CA (certificate authority) that signs new user certificates for mTLS authentication.
<5> The Kubernetes `Secret` that contains the private key (`ca.key`) value of the CA that signs new user certificates for mTLS authentication.
<6> The label to identify the `KafkaUser` resources managed by the User Operator.
...
<8> The size of the controller event queue.
The size of the queue should be at least as big as the maximal amount of users you expect the User Operator to operate.
The default is `1024`.
<9> The size of the worker pool for reconciling the users.
Bigger pool might require more resources, but it will also handle more `KafkaUser` resources
The default is `50`.
<10> The size of the worker pool for Kafka Admin API and Kubernetes operations.
Bigger pool might require more resources, but it will also handle more `KafkaUser` resources
The default is `4`.
<11> The level for printing logging messages.
You can set the level to `ERROR`, `WARNING`, `INFO`, `DEBUG`, or `TRACE`.
<12> Enables garbage collection (GC) logging.
The default is `true`.
<13> The validity period for the CA.
The default is `365` days.
<14> The renewal period for the CA. The renewal period is measured backwards from the expiry date of the current certificate.
The default is `30` days to initiate certificate renewal before the old certificates expire.
<15> (Optional) The Java options used by the JVM running the User Operator
<16> (Optional) The debugging (`-D`) options set for the User Operator
<17> (Optional) Prefix for the names of Kubernetes secrets created by the User Operator.
<18> (Optional) Indicates whether the Kafka cluster supports management of authorization ACL rules using the Kafka Admin API.
...
The default is `true`.
<19> (Optional) Semi-colon separated list of Cron Expressions defining the maintenance time windows during which the expiring user certificates will be renewed.
<20> (Optional) Configuration options for configuring the Kafka Admin client used by the User Operator in the properties format.
<21> (Optional) Regular expression to configure users for which any existing ACLs, Quotas and SCRAM-SHa credentials will be ignored.
This option is useful when you want to configure the User Operator to ignore users that are managed through another mechanism.
<22> (Optional) Controls automatic creation of PKCS12 stores in the Kafka user `Secret` resources.
The default is `true`.
...
. If you are using cert-manager to issue certificates, specify the cert-manager issuer.
Otherwise, go to the next step.
+
...
# ....
env:
  - name: STRIMZI_CERT_MANAGER_ISSUER_NAME <1>
    value: my-cluster-issuer
  - name: STRIMZI_CERT_MANAGER_ISSUER_KIND <2>
    value: ClusterIssuer
  - name: STRIMZI_CERT_MANAGER_ISSUER_GROUP <3>
    value: cert-manager.io
# ..."
----
<1> Name of the cert-manager `Issuer` or `ClusterIssuer` to use for `KafkaUser` certificates.
<2> Kind of the cert-manager issuer. Must be `Issuer` or `ClusterIssuer`.
<3> API group of the cert-manager issuer. Default value is `cert-manager.io`.
