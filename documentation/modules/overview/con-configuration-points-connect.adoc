...
NOTE: You can link:{BookURLConfiguring}#assembly-loading-config-with-providers-str[load confidential configuration values for a connector^] from Kubernetes Secrets or ConfigMaps.
...
* link:{BookURLConfiguring}#assembly-loading-config-with-providers-str[Loading configuration values from external sources^]
