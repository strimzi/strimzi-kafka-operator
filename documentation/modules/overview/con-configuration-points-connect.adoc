...
Using this resource, you can build a container image that includes the connector plugins used by worker pods.
You can use the resource to configure the worker pods too.
...
When you deploy Kafka Connect using the `KafkaConnect` resource, you specify bootstrap server addresses (in `spec.bootstrapServers`) for connecting to the Kafka cluster.
...
.Worker configuration that must be unique for each Kafka Connect cluster
[source,yaml,subs="attributes+"]
----
apiVersion: {KafkaConnectApiVersion}
kind: KafkaConnect
...
Connector plugins provide the implementation to create connector instances.
When a plugin is instantiated, configuration is provided for connection to an external data system.

After connector plugins have been added to the container image used for the worker pods in a deployment,
...
Connector plugins provide the implementation for connecting to specific types of external data system.
...
You can create a custom Kafka Connect image that includes your choice of connector plugins.
...
To create the container image automatically, you specify and deploy plugins for your Kafka Connect cluster using the `build` property of the `KafkaConnect` resource.
...
If enabled in your `KafkaConnect` configuration, you must use the `KafkaConnector` resource to define and manage connectors.
