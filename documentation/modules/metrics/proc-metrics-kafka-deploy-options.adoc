...
spec:
  ...
  kafkaExporter:
    ...
    template: <7>
      deployment:
        deploymentStrategy: RollingUpdate <8>
      pod:
        metadata:
          labels:
            label1: value1
...
<8> link:{BookURLUsing}#con-common-deployment-strategy-reference[Deployment strategy type].
<9> link:{BookURLUsing}#con-common-configuration-healthchecks-reference[Healthcheck readiness probes].
<10> link:{BookURLUsing}#con-common-configuration-healthchecks-reference[Healthcheck liveness probes].
