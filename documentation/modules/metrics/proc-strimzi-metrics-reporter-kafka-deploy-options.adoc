...
If you omit `allowList`, Strimzi applies a curated default set of patterns for the component instead of collecting all metrics.
For `Kafka`, the default set is role-specific: broker nodes receive broker metrics, controller nodes receive controller metrics, and nodes with both roles receive the union of the two.
This ensures each node only exposes the metrics relevant to it.
