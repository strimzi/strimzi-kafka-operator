...
** Configure in the `KafkaConnect`, or `KafkaMirrorMaker2` resources for `oauth` authentication.
...
Alternatively, copy the example configuration in `kafka-bridge-metrics.yaml` to your own `KafkaBridge` resource.

.Copying the example configuration
[source,shell]
----
kubectl edit kafkaBridge <kafka_configuration_file>
----

Copy the `metricsConfig` property and the `ConfigMap` it references to your `KafkaBridge` resource.

.Example metrics configuration for KafkaBridge
[source,yaml,subs="+quotes,attributes"]
----
apiVersion: {KafkaApiVersion}
kind: KafkaBridge
