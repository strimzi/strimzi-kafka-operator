...
* *Main goals* are inherited from Cruise Control and typically used as the default set of goals, with some preset as hard goals.
The goals can be customized to specify a subset of all available goals provided by Cruise Control.
...
* `KafkaRebalance` resource for user-provided goals.
...
You can override default goals by setting user-provided optimization goals in a `KafkaRebalance` resource.
...
User-provided optimization goals narrow down the configured default goals for specific optimization proposals.

Configure user-provided goals in `KafkaRebalance.spec.goals`:

* Specify a subset of main optimization goals for customization.

For example, you can optimize topic leader replica distribution across the Kafka cluster without considering disk capacity or utilization by defining a single user-provided goal.
...
IMPORTANT: Ensure that the main `goals`, `default.goals`, and (unless `skipHardGoalCheck` is set to `true`) user-provided `spec.goals` include all hard goals specified in `hard.goals` to avoid errors when generating optimization proposals. 
Hard goals must be included as a subset in the main, default, and user-provided goals.

.Example `KafkaRebalance` configuration for user-provided goals
[source,yaml,subs="attributes+"]
----
...
If `skipHardGoalCheck: true` is specified in the `KafkaRebalance` custom resource, Cruise Control does not verify that the user-provided goals include all the configured hard goals. 
...
However, any hard goals included in the user-provided goals will still be treated as hard goals by Cruise Control, even with `skipHardGoalCheck: true`.
