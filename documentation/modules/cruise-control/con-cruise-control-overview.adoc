...
= How Cruise Control rebalances a Kafka cluster
...
However, the Anomaly Detector's goal violation detection is used by the xref:proc-automating-rebalances-imbalance-{context}[auto-rebalance on imbalance] feature to trigger rebalances when cluster imbalances are detected.
...
== Assessing optimization proposals

Optimization proposals summarize the expected impact of rebalancing a Kafka cluster based on the defined optimization goals, assessed in a specific order of priority. 
You can approve or reject proposals and rerun them with adjusted goals if needed.

Optimization proposals are also a useful way to verify that a rebalance can succeed with the configured optimization goals and constraints.
They show whether Cruise Control can generate a valid rebalance plan and provide an indication of how the rebalance is expected to affect the Kafka cluster.
...
. If approved, the Executor recomputes the optimization using the latest cluster state and rebalances the Kafka cluster based on the configured optimization goals.
This involves reassigning partitions and redistributing workload across brokers.

.Cruise Control optimization process
image::kafka-concepts-cruise-control.png[Cruise Control process]

Optimization proposals comprise a list of partition reassignment mappings that describe the expected scope of a rebalance.
When you approve a proposal, Cruise Control initiates a cluster rebalance based on the current cluster state.
...
IMPORTANT: An optimization proposal reflects the state of the Kafka cluster at the *time the proposal is generated*.
...
This ensures the rebalance is based on the most recent cluster metrics.
As a result, the movements executed during the rebalance might differ from those shown in the proposal summary, especially on busy or rapidly changing clusters. The overall optimization outcome is typically similar if approval happens shortly after the proposal is generated.
...
The information provided is a summary of the optimization proposal generated for the current cluster state.
...
Use the summary to assess the expected impact of a rebalance and decide whether to approve it.
...
Cruise Control starts a cluster rebalance operation based on the configured optimization goals and the latest cluster state.
...
Compare the balancedness scores to see whether the Kafka cluster could be more balanced.
...
To save time, you can automate the approval step for running a cluster rebalance.
With automation, when an optimization proposal is generated, Cruise Control immediately starts a cluster rebalance.
...
You can still check the status of the rebalance when using automatic approval.
