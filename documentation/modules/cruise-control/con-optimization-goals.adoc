...
NOTE: If you change the inherited main optimization goals or default goals, you must ensure that the hard goals, if configured in the `hard.goals` property in `Kafka.spec.cruiseControl.config`, are a subset of the main optimization goals and default goals that you configured. Otherwise, errors will occur when generating optimization proposals.
