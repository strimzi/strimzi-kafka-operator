...
You configure optimization goals in `Kafka` and `KafkaRebalance` custom resources. Cruise Control has configurations for link:#hard-soft-goals[_hard_] optimization goals that must be satisfied, as well as link:#master-goals[_master_], link:#default-goals[_default_], and link:#user-provided-goals[_user-provided_] optimization goals. 
Optimization goals are subject to any link:#capacity-configuration[_capacity limits_] on broker resources.
