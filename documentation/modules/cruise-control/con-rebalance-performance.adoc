...
To preform an intra-broker disk balance and only an intra-broker disk balance set `rebalanceDisk` to `true` under the `KafkaRebalance.spec`.
When setting `rebalanceDisk` to `true`, do not set a `goals` field in the `KafkaRebalance.spec`, Cruise Control will automatically set the intra-broker goals and ignore the inter-broker goals.
Cruise Control does not prefrom inter-broker and intra-broker balancing at the same time.
