...
<11> Format type of the data for the API user configuration, currently only Jetty's HashLoginService format is supported since that is the format which Cruise Control uses for its API user configuration.
<12> Reference to secret containing API user configuration.
