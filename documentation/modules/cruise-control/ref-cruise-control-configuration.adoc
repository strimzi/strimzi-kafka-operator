...
[discrete]
== Disabling Cruise Control REST API security

The Cruise Control REST API is secured with authentication and authorization to protect the cluster against potentially destructive Cruise Control operations, such as decommissioning Kafka brokers.

We recommend that Cruise Control in Strimzi is **only used with security enabled**.
You should not disable the built-in authentication and authorization settings described below.
