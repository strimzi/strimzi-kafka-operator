...
An optimization proposal consists of two main components, the _summary_ and the _broker load_.
The summary section provides an overview of the optimization proposal and from this you can gain an idea of the scale of the changes involved in the proposed rebalance.
The broker load section allows you to see the impact of the proposed rebalance on each of the brokers in the cluster.
...
In the broker load section, each broker is listed under its broker ID number. 
...
The first is the metric value before the optimization proposal is applied, the second is the expected value of the metric after the application of the proposal and the last value is the difference between the first two values (after - before).
...
m| diskUsedMB                  | The absolute disk usage in megabytes.
