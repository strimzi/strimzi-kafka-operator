...
Each optimization proposal is based on the set of xref:con-optimization-goals-{context}[optimization goals] that was used to generate it, subject to any configured x-ref:#capacity-configuration[capacity limits on broker resources].

An optimization proposal is contained in the `status.summary` property of a `KafkaRebalance` custom resource. 
