...
If you want, an optimization proposal can be automatically approved.
...
Not setting the annotation at all or setting it to `false` leaves the optimization proposal with the need to be manual approved by the user.

.Example of rebalance request with auto-approval mechanism enabled
[source,yaml]
----
