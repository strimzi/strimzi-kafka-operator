...
When running Kafka 4.3 or later, brokers scheduled for removal are automatically _cordoned_ during a scale-down operation.
Cordoning sets the `cordoned.log.dirs` broker configuration to prevent new partition assignments to the broker while partitions are being moved off it.
...
A third mode, `imbalance`, is also available for automatically rebalancing when Cruise Control detects goal violations in the cluster.
For more information, see xref:proc-automating-rebalances-imbalance-{context}[].
  ...
  autoRebalance:
    ...
    modes: # <2>
      ...
      - mode: remove-brokers
        brokers: <broker_ids>       
----
<1> The state of the rebalance, which shows `RebalanceOnScaleUp` when adding brokers, `RebalanceOnScaleDown` when removing brokers, and `RebalanceOnImbalance` when rebalancing due to detected goal violations.
Scale-down operations take precedence over scale-up, and both take precedence over imbalance rebalancing.
