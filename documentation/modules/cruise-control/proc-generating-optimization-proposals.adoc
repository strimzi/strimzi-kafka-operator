...
Review the optimization proposal to assess the expected impact of a cluster rebalance and decide whether to proceed.
The proposal reflects the state of the Kafka cluster at the time it is generated.
The executed rebalance might differ from the proposal if cluster conditions change before approval.
...
----
+
The properties in the `Optimization Result` section describe the expected scope of a cluster rebalance.
...
It is like having a single _virtual_ CPU per broker, which has the cycles of the CPUs configured in `KafkaNodePool.spec.resources.limits.cpu`.
