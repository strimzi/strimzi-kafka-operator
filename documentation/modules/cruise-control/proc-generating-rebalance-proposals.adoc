...
When you create or update a `KafkaRebalance` resource, Cruise Control generates an _optimization proposal_ in JSON.
This summarizes an optimal mapping of partitions to brokers for the Kafka cluster, based on the optimization goals that were used, along with statistics on the cluster rebalance.
