...
When you create or update a `KafkaRebalance` resource, Cruise Control generates an _optimization proposal_ for the Kafka cluster based on the optimization goals that were used.
The proposal summarizes the following information, in JSON:

* An optimal mapping of partitions to brokers

* Changes to the partition leaders

* Statistics on the pending cluster rebalance

You can analyze the summary and decide whether to implement the optimization proposal. 
...
.. To use the default optimization goals defined in the `Kafka` resource, leave the `spec` property empty:
+
...
.. To use one or more user-provided optimization goals instead of the default goals, edit the `spec.goals` property.
