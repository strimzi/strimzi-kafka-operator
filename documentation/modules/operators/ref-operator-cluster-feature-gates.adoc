...
When enabled, the Connect Build feature will use Buildah instead of Kaniko during the image build process.
Buildah - as Kaniko - will be used only on Kubernetes, on OpenShift the mechanism remains the same.
When Buildah is used, there is no change in behavior - the only change is in the additional options that users can specify in the `build` section of KafkaConnect's `.spec`.
These are split into two groups - one for build phase and one for push phase - which you can configure using `additionalBuildOptions` and `additionalPushOptions` fields.
