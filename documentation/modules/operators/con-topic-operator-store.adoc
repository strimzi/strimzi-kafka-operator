...
[role="_abstract"]
The Topic Operator uses persistent storage to store topic metadata describing topic configuration as key-value pairs.
The _topic store_ is based on the Kafka Streams key-value mechanism, which uses persistent storage with local and remote components.

Topic metadata is stored and accessed locally in-memory.
Updates from operations applied to the local in-memory topic store are persisted to a backup topic store on disk.
...
but should the in-memory topic store crash it is automatically repopulated from the persistent storage.
...
Additionally, you must delete the topics created for the topic store using an `exec` command, specifying the bootstrap address of the Kafka cluster and the name of a broker pod.
...
NOTE: When scaling a Kafka cluster, the topic store topics must also be deleted so that partitions can be reassigned.

== Scaling and topics

The recommended configuration for topics is to have a topic replication factor of 3, so that there are 2 in-sync replicas on other brokers.
If a broker is unavailable, at least one other broker is in-sync.
...
When scaling Kafka clusters by adding or removing brokers, replicas are not reassigned automatically.
...
Alternatively, though Cruise Control cannot change the replication factor for topics,
