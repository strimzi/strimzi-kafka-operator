...
NOTE: If a GA feature has been enabled by default for some time, its feature gate might be removed.  
...
Using control plane listeners can provide increased performance because important controller connections, such as partition leadership changes, are not delayed by data replication across brokers.
