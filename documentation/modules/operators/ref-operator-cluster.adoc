...
To modify a feature gate's default state, use the `STRIMZI_FEATURE_GATES` environment variable in operator's configuration.  
You can modify multiple feature gates in one environment variable.
...
We might remove a GA stage feature gate that has been enabled by default for some time, so disabling the feature will no longer be possible. 
Alpha and beta stage features will be removed if they do not prove to be useful.

.Supported feature gates and the Strimzi versions when they moved to alpha, beta, or GA
...
Specify a comma-separated list of feature gate names and prefixes for the environment variable. 
...
You can use the `ControlPlaneListener` feature gate to change the communication paths used for inter-broker communications.

When `ControlPlaneListener` is enabled, Kafka brokers use a dedicated internal _control plane listener_ for controller connections (that is, traffic from the Kubernetes control plane) between brokers. 
The control plane listener uses port 9090. 
Data connections (data replication between the Kafka brokers) continues to use the internal listener on port 9091. 

When `ControlPlaneListener` is disabled, controller and data connections go through the same internal listener on port 9091. 
This was the default behavior before the feature gate was introduced.

Control plane listeners might improve performance because important requests from the control plane, such as partition leadership changes, are not delayed by data plane traffic, such as data replication.
