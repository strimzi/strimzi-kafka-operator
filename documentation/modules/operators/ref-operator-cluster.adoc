...
The `STRIMZI_OPERATOR_NAMESPACE` environment variable is configured by default, and uses the Kubernetes Downward API to find which namespace the Cluster Operator is running in.
If the Cluster Operator is running in the same namespace, only local access is required, and allowed by Strimzi.
