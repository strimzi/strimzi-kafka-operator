...
When enabled, the `NetworkPolicyGeneration` feature gate enables Strimzi to create default Network Policies for the services it creates.
This feature gate is currently in the beta phase and is enabled by default.
To disable it, use `-NetworkPolicyGeneration` in the `STRIMZI_FEATURE_GATES` environment variable in the Cluster Operator configuration.
