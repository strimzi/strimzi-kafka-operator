...
Strimzi operators supports feature gates to gate selected functionality and switch it on or off.
Each feature gate has a default state (enabled or disabled by default) and users are to manually enable or disable them.
The feature gates always apply for the whole operator and alter its behavior.
...
Alpha stage indicates that the feature might be unstable (i.e. it might change in the future), experimental or not sufficiently tested for production use.
Beta stage indicates a that the feature is already well tested and its functionality should most probably not change.
GA means that the feature is stable and should not change anymore.
GA features might optionally also change to always enabled features.
...
You can configure the feature gates using the environment variable `STRIMZI_FEATURE_GATES`.
It should contain a comma separate list of the feature gate names prefixed with `+` or `-` for enabling or disabling the feature gate.
...
Feature gates can be in 3 different phases indicating their maturity level:
* _Alpha_ features

=== Control Plane Listener

When enabled, the `ControlPlaneListener` feature gate will configure Kafka brokers to use a separate listener for controller connections and for data connections (replication).
This feature gate is currently in the alpha phase and is disabled by default.
To enabled it, use `+ControlPlaneListener` in the `STRIMZI_FEATURE_GATES` environment variable in the Cluster Operator configuration.
