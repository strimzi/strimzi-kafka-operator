...
Disables or enables feature gates to activate new features or change the operator behavior.
For more details about the feature gates, see xref:ref-operator-cluster-feature-gates-{context}[].

[id='ref-operator-cluster-feature-gates-{context}']
== Feature gates

Strimzi operators support feature gates to turn functionality on and off.
When applied, feature gates alter the behavior of the operator.
You can change the default state for each feature gate to enable or disable them.

Feature gates have 3 stages of maturity:
...
Alpha stage features might be unstable, subject to change, experimental, or not sufficiently tested for production use.
Beta stage features are already well tested and their functionality is not likely to change.
GA means that a feature is stable and should not change.
GA features might also become features that are always enabled.
In this case, the feature gate is removed and it will not be possible to disable the feature.
Alpha and beta features are removed if they do not prove to be useful.

.List of supported feature gates and Strimzi versions when they moved into the alpha, beta, or GA stage
...
You configure feature gates using the environment variable `STRIMZI_FEATURE_GATES`.
The configuration contains a comma-separated list of the feature gate names.
A prefix of `+` enables the feature gate. A prefix of `-`  disables the feature gate.
...
When enabled, the `ControlPlaneListener` feature gate configures Kafka brokers to use a separate listener for controller connections and for data connections (replication).
This feature gate is currently in the alpha phase and is disabled by default.
To enable it, use `+ControlPlaneListener` in the `STRIMZI_FEATURE_GATES` environment variable in the Cluster Operator configuration.
This feature gate has to be disabled when upgrading from Strimzi 0.22 and earlier or when downgrading to Strimzi 0.22 and earlier.

NOTE: The `ControlPlaneListener` feature gate was introduced in Strimzi 0.23.0 and is expected to remain in the alpha phase for a number of releases before it moves to the beta phase and is enabled by default.

=== Service Account patching feature gate

The default behavior of the Strimzi Cluster Operator is that once the Service Account exists, it is never updated.
When the `ServiceAccountPatching` feature gate is enabled, the operator will patch the Service Accounts and apply any desired changes to the Service Account configuration in every reconciliation.
Tsi for example allows changing labels or annotations after the operands are already created.
This feature gate is currently in the alpha phase and is disabled by default.
To enable it, use `+ServiceAccountPatching` in the `STRIMZI_FEATURE_GATES` environment variable in the Cluster Operator configuration.
