...
Enables or disables feature gates that activate new features or change the behavior of the operator.
...
You can use the `STRIMZI_FEATURE_GATES` environment variable to enable or disable one or more feature gates by changing their default state.
...
If a GA stage feature has been enabled by default for some time, its feature gate might be removed. 
In this case, it will not be possible to disable the feature. 
Alpha and beta features will be removed if they do not prove to be useful.
...
You configure feature gates using the `STRIMZI_FEATURE_GATES` environment variable in the configuration of the relevant operator.
The configuration contains a comma-separated list of feature gate names and prefixes.
...
You can use the `ControlPlaneListener` feature gate to change the communication paths used for inter-broker communication.

When enabled, Kafka brokers use a separate internal _control plane listener_ for controller connections (that is, traffic from the Kubernetes control plane). 
The control plane listener uses port 9090. 
Data connections (data replication between the Kafka brokers) continues to use the existing internal listener on port 9091. 

Control plane listeners might improve performance because important requests from the control plane, such as partition leadership changes, are not delayed by data plane traffic.

The `ControlPlaneListener` feature gate is in the alpha phase and disabled by default. 
...
This feature gate must be disabled when upgrading from Strimzi 0.22 and earlier or when downgrading to Strimzi 0.22 and earlier.

NOTE: The `ControlPlaneListener` feature gate was introduced in Strimzi 0.23.0 and is expected to remain in the alpha phase for a number of releases before moving to the beta phase.
