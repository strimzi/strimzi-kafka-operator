...
The boolean value, controlling whether Strimzi should generate Network Policy files using the operator.

By default, this is enabled, causing Network policies to be generated which allow Kafka connections between the relevant components involved.

However, if the user desires custom network policies, they may disable this generation and take the responsibility for maintaining connections between the components.
