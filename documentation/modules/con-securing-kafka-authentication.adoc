...
Listener configuration is specified as an array within a `Kafka` resource, which makes configuration very flexible,
and allows you to configure multiple listeners.

For clients inside the Kubernetes cluster, you can create `plain` (without encryption) or `tls` _internal_ listeners.

For clients outside the Kubernetes cluster, you create _external_ listeners,
and specify a connection mechanism, which can be `nodeport`, `loadbalancer`, `ingress` or `route` (on OpenShift).
