...
<12> xref:ref-kafka-broker-configuration-{context}[SSL properties for external listeners to run on a specific _cipher suite_ for a TLS version].
...
<22> Cruise Control configuration, which is used xref:proc-deploying-cruise-control-str[to monitor and balance data across the Kafka cluster].
