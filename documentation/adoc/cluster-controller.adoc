...
These permissions are described in terms of Kubernetes/OpenShift role-based access controls.
...
The Deployment of the controller then needs to specify this in the `serviceAccountName` of it's `template`´s
 `spec`:
...
should operate in. This is a required environment variable, but the exmple YAML resources
