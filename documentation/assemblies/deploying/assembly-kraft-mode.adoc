...
The `Kafka` resource using KRaft mode must also have the annotation `strimzi.io/kraft: enabled`.
...
An active lead controller manages updates to the metadata log, and follower controllers replicate these updates. 
...
Brokers store the metadata log passively to stay up-to-date with the cluster's state.
...
In a typical configuration, a Kafka cluster includes a quorum of controller and broker nodes for fault tolerance and high availability. 
...
In a production environment, avoid using nodes that operate in a combined mode. 
...
.Example cluster with nodes that combine roles
