...
For manual rolling updates, annotate the individual `Pod` resources directly.
This avoids the risk of a `StrimziPodSet` annotation being overwritten during reconciliation before the rolling update is triggered.
