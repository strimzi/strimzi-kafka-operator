...
[role="_abstract"]
If your Kafka broker and ZooKeeper pods were deployed by Strimzi, you can use the Strimzi Drain Cleaner tool to evict them from nodes.
You might want to evict pods for voluntary disruptions during maintenance or upgrade.
...
As broker nodes are drained consecutively, the Cluster Operator waits until topics are synchronized.
...
ifdef::Downloading[]
The Strimzi Drain Cleaner deployment files are accessible from the
link:https://github.com/strimzi/drain-cleaner[GitHub repository^].
endif::Downloading[]
