...
You perform rebalances through the `KafkaBalance` resource, which generates and applies optimization proposals.  
...
* *User-defined* goals are a subset of default goals that are configured for generating a specific optimization proposal. 
--
...
+
--
* *`full` mode* is the default mode and runs a full rebalance.
* *`add-brokers` mode* is the mode you use after adding brokers when scaling up a Kafka cluster.  
* *`remove-brokers`* mode is the mode you use before removing brokers when scaling down a Kafka cluster.
--
