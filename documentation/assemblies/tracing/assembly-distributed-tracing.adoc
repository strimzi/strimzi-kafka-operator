...
Strimzi uses OpenTracing and Jaeger Tracing projects to implement tracing support
It is enabled by setting the `.spec.tracing.type` property to `jaeger` in the component configuration.
...
In the future, we plan to introduce a new tracing implementation based on the OpenTelemetry project.
====
