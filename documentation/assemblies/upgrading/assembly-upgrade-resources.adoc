...
[role="_abstract"]
Before upgrading Strimzi to {ProductVersion}, you must ensure that your custom resources are using API version `{ApiVersion}`.
You can do this any time after upgrading to Strimzi {ConvertAfterProductVersion}, but the upgrades must be completed before upgrading to Strimzi {ConvertBeforeProductVersion} or newer.

IMPORTANT: Upgrade of the custom resources to `{ApiVersion}` _must_ be performed before xref:assembly-upgrade-{context}[upgrading the Cluster Operator], so the Cluster Operator can understand the resources.
