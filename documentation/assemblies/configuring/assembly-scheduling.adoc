...
[role="_abstract"]
To avoid performance degradation caused by resource conflicts between applications scheduled on the same Kubernetes node, it is advisable to schedule Kafka pods separately from critical workloads. 
