...
* Tiered storage (Technology preview)

To configure non-tiered storage, you specify `storage` properties in the custom resource of the component. 
The storage type is set using the `storage.type` property.

Support for tiered storage is being introduced as a technology preview.
