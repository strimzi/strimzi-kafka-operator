...
Even if you start with only one volume, using JBOD allows for future scaling by adding more volumes as needed.

NOTE: Persistent, ephemeral, and JBOD storage types cannot be changed after a Kafka cluster is deployed, though you can create and migrate to node pools with new storage specifications.
