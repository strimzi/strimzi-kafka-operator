// This assembly is included in the following assemblies:
//
// assembly-tracing.adoc
[id='assembly-instrumenting-kafka-clients-tracers-{context}']
= Instrumenting Kafka clients with tracers

This section describes how to instrument Kafka Producer, Consumer, and Streams API applications for distributed tracing.

include::../modules/proc-instrumenting-producers-consumers-opentracing.adoc[leveloffset=+1]

include::../modules/proc-instrumenting-kafka-streams-with-tracers.adoc[leveloffset=+1]
