...
It is possible to override the auto assigned node ports for bootstrap service and brokers, and advertised host and port for each broker.

.An example where the external listener is configured with overrides for node ports and advertised addresses
[source,yaml,subs="attributes+"]
----
# ...
listeners:
  external:
    ...
    overrides:
      ...
      brokers:
      - broker: 0
        advertisedHost: example.host.1
        advertisedPort: 32401
        nodePort: 32401
      - broker: 1
        advertisedHost: example.host.2
        advertisedPort: 32402
        nodePort: 32402
      - broker: 2
        advertisedHost: example.host.3
        advertisedPort: 32403
        nodePort: 32403
# ...
----
