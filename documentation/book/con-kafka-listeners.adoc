...
{ProductName} is using the TLS passthrough feature of the {NginxIngressController}.
Make sure TLS passthrough is enabled in your {NginxIngressController} deployment.
For more information abotu enabling TLS passthrough see {NginxIngressControllerTLSPassthrough}.
...
You have to specify the hostnames which should be used by the bootstrap and per-broker services in the `configuration` section.
