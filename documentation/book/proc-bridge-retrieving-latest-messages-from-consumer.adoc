...
Next, retrieve the latest messages from a Kafka Bridge consumer by requesting data from the link:https://strimzi.io/docs/bridge/latest/#_poll[records^] endpoint. For production use, HTTP clients can retrieve messages repeatedly or as a one-off operation. 
