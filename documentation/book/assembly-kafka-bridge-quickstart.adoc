...
This quick start describes how to produce and consume messages in JSON format, not binary. 

* When producing messages to topics and partitions, the following `Content-Type` header is used:
+
...
* When creating a Kafka Bridge consumer and performing consumer operations, the following `Content-Type` header is used:
+
[source,http,subs=attributes+]
----
...
For more information, see xref:con-requests-kafka-bridge-{context}[Requests to the {ProductName} Kafka Bridge].
