...
* xref:downloads-str[{ProductName} is installed].
* Cluster administrator access to a local or remote Kubernetes cluster (or you are a designated xref:assembly-getting-started-strimzi-admin-str[Strimzi Administrator]). 
* A running Kafka cluster, deployed by the Cluster Operator, in a Kubernetes namespace.
* The xref:deploying-the-topic-operator-using-the-cluster-operator-str[Entity Operator is deployed] and running as part of the Kafka cluster.  
