...
+
ifdef::Kubernetes[]
In {KubernetesName} this can be done using `kubectl create`
+
.Creating the `KafkaTopic` resource using `kubctl create`
[source,shell,subs=+quotes]
----
kubectl create -f _<your-file>_
----
+
In {OpenShiftName} this can be done using `oc create`
endif::Kubernetes[]
ifndef::Kubernetes[]
...
+
.Creating the `KafkaTopic` resource using `oc create`
[source,shell,subs=+quotes]
----
...
* For more information about the API for `KafkaTopics`, see xref:type-KafkaTopic[`KafkaTopic` API documentation]
