...
The `{KafkaApiVersionPrev}` API version has been deprecated for this release.

IMPORTANT: The upgrade of resources _must_ be performed before xref:assembly-upgrade-{context}[], so the Cluster Operator can understand the CRDs.
