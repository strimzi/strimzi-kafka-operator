...
The `rack` object has one mandatory field named `topologyKey`, which needs to match one of the labels assigned to the {ProductPlatformName} cluster nodes. The label is used by {ProductPlatformName} when scheduling the Kafka broker pods to nodes.
