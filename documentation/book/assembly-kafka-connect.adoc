...
Two built-in connectors are provided:
...
In {ProductName}, you can use the Cluster Operator to deploy a Kafka Connect or a Kafka Connect Source-2-Image (S2I) cluster to your {ProductPlatformName} cluster.

The Kafka Connect cluster is implemented as a `Deployment` with a configurable number of workers. The Kafka Connect REST API is available on port 8083, as the `kafka-connect` service.
