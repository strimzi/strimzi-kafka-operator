...
The provided `alerting-rules.yaml` file creates a `ConfigMap` with sample alerting rules.
...
Other than a server for scraping metrics, Prometheus provides an alert manager component for alerting.
It is possible to declare alerting rules in order to notify the user about specific conditions in the metrics.
Notifications can be delivered via email, on Slack channels and other built-in mechanisms.
...
The provided configuration configures the alert manager to send notification to Slack channels.
Before deploying the alert manager it is needed to update a couple of parameters.
The `slack_api_url` needs to be updated with the actual value of the Slack API URL related to application for the Slack workspace.
The `channel` field needs to be updated with the actual Slack channel on which sending the notification.
