...
NOTE: When both the `STRIMZI_IMAGE_PULL_SECRETS` environment variable in Cluster Operator and `imagePullSecrets` option are specified, only the `imagePullSecrets` will be used and the `STRIMZI_IMAGE_PULL_SECRETS` will be ignored.
