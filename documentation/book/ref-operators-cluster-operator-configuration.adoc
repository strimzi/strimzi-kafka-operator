...
The secrets referenced here should contain the credentials to the container registries where the container images should be pulled from.
The secrets will be used in the `imagePullSecrets` field for all `Pods` created by the Cluster Operator.
Changing this list will result in a rolling update of all your Kafka, Kafka Connect, and Kafka Mirror Maker clusters.