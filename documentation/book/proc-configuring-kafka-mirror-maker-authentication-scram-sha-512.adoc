...
Because Kafka Mirror Maker connects to two Kafka clusters (source and target), it is possible to configure SCRAM-SHA-512 authentication for one or both of them.
...
The same steps can be done for configuring SCRAM-SHA-512 authentication on the producer side for connecting to the target Kafka cluster.
