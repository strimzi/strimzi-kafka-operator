...
WARNING: Increasing of volume size for existing {ProductName} clusters is supported only on {ProductPlatformName} versions which support persistent volume resizing and with storage classes which support volume expansion.
For other versions of {ProductPlatformName} and storage classes which do not support volume expansion, you must decide the necessary storage size before deploying the cluster.
Shrinking of existing volumes is not possible.
...
{ProductName} allows to specify different storage class for each broker.
This is useful for example when the storage classes are restricted to different availability zones / datacenters.
You can use the `overrides` field for this purpose.

.Example of {ProductName} cluster using the storage class overrides
[source,yaml,subs="attributes+"]
----
...
In the example above, the default storage class is `my-storage-class`.
But thanks to the configured `overrides` property, the broker volumes will use following storage classes:

* Persistent volumes of broker 0 will use `my-storage-class-zone-1a`
* Persistent volumes of broker 1 will use `my-storage-class-zone-1b`
* Persistent volumes of broker 2 will use `my-storage-class-zone-1c`

The `overrides` property can be currently used only to override the storage class configuration.
