...
The persistent volume will be used by the Kafka brokers as log directory mounted into the following path:

`/var/lib/kafka/data/kafka-log_idx_`::
Where `_idx_` is the Kafka broker pod index. E.g. `/var/lib/kafka/data/kafka-log0`.