...
Decreasing the size of existing volumes is not possible.
...
{ProductName} allows you to specify a different storage class for each broker.
This is useful, for example, when storage classes are restricted to different availability zones or datacenters.
...
The `overrides` property is currently used only to override the storage class configuration.
