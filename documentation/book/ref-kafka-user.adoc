...
Simple Authorization supports 4 different resource types:
+
* Topics
* Consumer Groups
* Clusters
...
Use `topic` for Topics, `group` for Consumer Groups, `cluster` for clusters, and `transactionId` for Transactional IDs.
+
Topic, Group, and Transactional ID resources additionally allow to specify the name of the resource for which the rule applies.
