...
More info about the topic operator in the related xref:deploying-the-topic-operator-using-the-cluster-operator-str[Topic Operator] documentation page.
...
See the reference about xref:type-EphemeralStorage-reference[ephemeral storage] and xref:type-PersistentClaimStorage-reference[persistent storage] for further details.
...
It is possible to configure {ProductPlatformName} resource limits and requests for the `kafka`, `zookeeper`, and `topicOperator` objects in the `Kafka` resource and for the `spec` object of the `KafkaConnect` resource.
...
=======
>>>>>>> Rewrite the cluster-operator section into modules
...
.Example fragment of a `Kafka` resource with a Topic Operator configuration
[source,yaml]
----
apiVersion: {KafkaApiVersion}
kind: Kafka
...
More information about these configuration parameters in the related xref:deploying-the-topic-operator-str[] documentation page.

<<<<<<< HEAD
[[kafka_connect_config_map_details]]
==== Kafka Connect

The operator watches for the `KafkaConnect` resource in order to find and get configuration for a Kafka Connect cluster to deploy.
...
`healthcheck-delay`::
The initial delay in seconds for the liveness and readiness probes for each Kafka Connect worker node.
Default is 60.
...
Before starting the build, a directory with all the Kafka Connect plugins which should be added has to be created.
The plugins and all their dependencies can be in a single directory or can be split into multiple subdirectories.
For example the following directory structure would be created in order to add the MySQL and Postgres CDC (change data capture) connectors provided by the link:http://debezium.io/[Debezium^] project:
...
no image is specified as the `kafka-image` in the xref:assembly-configuring-container-images-deployment-configuration-kafka[].
...
no image is specified as the `spec.kafka.tlsSidecar.image` in the xref:assembly-configuring-container-images-deployment-configuration-kafka[].
...
no image is specified as the `image` in the Kafka Connect cluster ConfigMap
...
no image is specified as the `spec.topicOperator.image` in the xref:assembly-configuring-container-images-deployment-configuration-kafka[].
...
no image is specified as the `zookeeper-image` in the xref:assembly-configuring-container-images-deployment-configuration-kafka[].
...
no image is specified as the `spec.zookeeper.tlsSidecar.image` in the xref:assembly-configuring-container-images-deployment-configuration-kafka[].
...
no image is specified as the `spec.topicOperator.tlsSidecar.image` in the xref:assembly-configuring-container-images-deployment-configuration-kafka[].
...
----
=======
>>>>>>> Annihilate last parts of clster-operator.adoc
