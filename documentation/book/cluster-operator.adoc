...
The full schema is described in xref:type-Kafka-{context}[].
...
See the reference about xref:type-EphemeralStorage-{context}[ephemeral storage] and xref:type-PersistentClaimStorage-{context}[persistent storage] for further details.
