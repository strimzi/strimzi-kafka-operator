...
* `[cluster-name]-zookeeper-config` ConfigMap which contains the Zookeeper ancillary configuration and is mounted as
a volume by the Zookeeper node pods
* `[cluster-name]-kafka-config` ConfigMap which contains the Kafka ancillary configuration and is mounted as
...
The setting can be done in one of two ways. Either by specifying the key value in JSON or by using a custom config map.
