...
The Docker image to use for the sidecar container which provides TLS support for Zookeeper nodes.
Default is determined by the value of the `<<STRIMZI_DEFAULT_TLS_SIDECAR_ZOOKEEPER_IMAGE,STRIMZI_DEFAULT_TLS_SIDECAR_ZOOKEEPER_IMAGE>>` environment variable of the Cluster Operator.
...
[[STRIMZI_DEFAULT_TLS_SIDECAR_IMAGE]] `STRIMZI_DEFAULT_TLS_SIDECAR_IMAGE`:: Optional, default `strimzi/zookeeper-stunnel:latest`.
