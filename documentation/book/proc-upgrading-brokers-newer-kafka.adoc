...
. Consult the table below and determine whether the new Kafka version has a different log message format version than the old version. 
+
...
----
+
NOTE: You must format the value of `log.message.format.version` as a string to prevent it being interpreted as a number.
+
...
<2> This is still the old version
...
Alternatively, if the old and new versions of Kafka have the same interbroker protocol version look in the Cluster Operator logs for a `INFO` level message like:
...
+
WARNING: You will not be able to downgrade after this step. If, for whatever reason, you need to revert the update at this point, follow the procedure xref:proc-downgrading-brokers-older-kafka-{context}[]. 
