...
{oauth} enables standardized token based authentication and authorization between applications, using a central {oauth} authorization server to issue tokens that grant limited access to resources.
{oauth} authorization is currently not supported.
...
The client presents an access token as a means of authenticating, which application servers can also use to find more information about the level of access granted.
