...
The Cluster Operator deploys one or more Kafka Mirror Maker instances, which can be used to replicate data between Kafka clusters.
Such a process is named "mirroring" for avoiding confusion with the Kafka partitions replication concept.
The Mirror Maker consumes from a source cluster and produce to a target cluster.
