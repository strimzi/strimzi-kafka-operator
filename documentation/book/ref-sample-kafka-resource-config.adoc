...
<14> Rack awareness is configured to xref:assembly-kafka-rack-{context}[to spread replicas across different racks]. A `topology` key must match the label of a cluster node.
