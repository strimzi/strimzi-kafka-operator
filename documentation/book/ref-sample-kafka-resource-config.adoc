...
<2> Kafka version, xref:assembly-upgrade-str[which can be upgraded with no downtime].
<3> Resources requests xref:ref-resource-limits-and-requests-{context}[specify the resources to reserve for a given container].
