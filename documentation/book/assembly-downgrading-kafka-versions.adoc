...
If the target downgrade version of Kafka has the same log message format version as the current verison, the Cluster Operator downgrades by performing a single rolling restart of the brokers.
...
In this case, the downgrade requires two rolling restarts of the brokers if the interbroker protocol of the two versions is different, or a single rolling restart if they are the same.
