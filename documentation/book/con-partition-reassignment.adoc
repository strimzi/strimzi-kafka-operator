...
To reassign a partition to a specific volume,, the `log_dirs` option has to be added to the _<PartitionObjects>_.
...
The `log_dirs` object should contain the same number log directories as is the number of replicas in the the `replicas` object.
