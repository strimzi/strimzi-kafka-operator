...
[id='deploying-the-topic-operator-standalone']
...
+
On {OpenShiftName} this can be done using `oc applydescribe:
endif::Kubernetes[]
ifndef::Kubernetes[]
...
* For more information about the environment variables used to configure the Topic Operator, see xref:ref-topic-operator-environment-str[]
* For more information about getting the Cluster Operator to deploy the Topic Operator for you, see xref:proc-deploying-the-topic-operator-using-the-cluster-operator-str[]