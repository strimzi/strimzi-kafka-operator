...
|====
|Field              |Description
|authorizer  1.2+<.<|Specifies the authorizer which should be used. Supported authoriizers are: `SimpleAclAuthorizer`.
|string (one of [SimpleAclAuthorizer])
|====
...
|====
|Field                  |Description
|authentication  1.2+<.<|Authentication configuration for Kafka's TLS listener
...
|====
|Field                           |Description
|tlsClientAuthentication  1.2+<.<|Configures TLS Client Authentication on port 9093. Valid values are `required`, `requested` and `none`. When not specified, the default value will be `none`.
|string (one of [requested, none, required])
|====
