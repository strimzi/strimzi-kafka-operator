...
Use these environment variables to configure a Jaeger tracer for client applications.
...
// We should make clear that these are not our variables but Jaegers and they are subject to change by Jaeger and ideally add a link to their docs for the description in case they change.
...
¦No
¦The host name and port to use if a Remote sampling strategy is selected.
