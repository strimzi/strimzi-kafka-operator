...
{ProductName} is designed to work on all types of Kubernetes cluster irrespective of distribution, from public and private clouds to local deployments intended for development.
{ProductName} supports a few features which are specific to OpenShift and OKD, where such integration benefits OpenShift users and cannot be implemented equivalently using standard Kubernetes.
