...
The Topic Operator is deployed as a process running inside an {ProductPlatformName} cluster. 
...
The `KafkaTopic` contains the topic configuration in a specific format, described in xref:type-KafkaTopic[`KafkaTopic` API reference].
