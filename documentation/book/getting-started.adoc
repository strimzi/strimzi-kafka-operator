...
For information on the logging options and examples of how to set logging, see xref:assembly-logging-deployment-configuration-kafka[] for Kafka.
...
=======
When using external ConfigMap remember to place your custom ConfigMap under `log4j2.properties` key.
>>>>>>> Add KAfka Connect TLS and Authentication support
