...
* A _reassignment JSON file_ of how partitions should be reassigned to brokers in the cluster once the broker(s) in the highest numbered `Pod(s)` have been removed.
...
You can verify this by checking that the broker's data log directory doesn't contain any live partition logs.
