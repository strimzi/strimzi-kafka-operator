...
But again, because of the privilege escalation prevention, the Cluster Operator cannot grant privileges it does not have itself (so it cannot, for example, create a `ClusterRoleBinding` to a `ClusterRole` which the Cluster Operator isn't already bound to via some other `ClusterRoleBinding`).
