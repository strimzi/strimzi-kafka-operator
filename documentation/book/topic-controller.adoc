...
The role of the topic controller is to keep a set of {ProductPlatformName} ConfigMaps describing Kafka topics in-sync with
...
. When creating a ConfigMap:
    * Remember that the name cannot be easily change.
...
Since the controller is intended to execute within {ProductPlatformName}, this can be achieved
by deleting the pod.
