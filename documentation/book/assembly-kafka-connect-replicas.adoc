...
Kafka Connect clusters can run with different number of nodes.
...
When the node where Kafka Connect is deployed to crashes, {ProductPlatformName} will automatically take care of rescheduling the KAfka Connect pod to different node.
