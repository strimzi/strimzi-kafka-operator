...
Next, create a Kafka Bridge consumer by using the link:https://strimzi.io/docs/bridge/latest/#_createconsumer[consumers/{groupid}^] endpoint. This consumer enables you to perform consumer operations in the Kafka cluster, such as subscribing to topics and retrieving messages from topics.
...
+
NOTE: The consumer will not commit offsets to the log automatically because the `enable.auto.commit` setting is `false`. You will commit the offsets for this consumer in a later procedure.
+
