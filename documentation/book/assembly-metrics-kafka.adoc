...
The Grafana dashboard provided {ProductName} is dependent on Prometheus JMX Exporter relabeling rules, which are defined for Kafka and ZooKeeper as `Kafka` resource configuration in an example `kafka-metrics.yaml` file.
