...
Strimzi uses a component called the Topic Controller to handle manage topics in the Kafka cluster. The Topic Controller
is deployed as a process running inside your Kubernetes or OpenShift cluster. To create a new Kafka topic, a ConfigMap
...
Strimzi allows to deploy the Topic Controller through the Cluster Controller. The cluster ConfigMap contains the Topic
Controller configuration for doing that.

Furthermore, Strimzi contains example YAML files for deploying the Topic Controller not through the Cluster Controller as well.
...
Of course, such a deployment comes "out of box" from deploying the Cluster Controller as explained in <<Cluster Controller>>.
...
By default, deploying the Topic Controller through the Cluster Controller needs the topic ConfigMap being labeled with
following labels:
...
Deploying the Topic Controller "manually" through the provided YAML files needs just the `strimzi.io/kind: topic` label.
