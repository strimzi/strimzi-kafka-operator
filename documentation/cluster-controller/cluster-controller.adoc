...
Kafka Connect cluster which connects to an existing Kafka cluster.

When the cluster controller is up and running, it starts to "watch" for ConfigMaps containing the Kafka or Kafka Connect
cluster configuration. Such ConfigMaps need to have a specific label which by default is `strimzi.io/kind=cluster` (as described later)
that can be changed through a corresponding environment variable.

When a new ConfigMap is created on the Kubernetes/OpenShift cluster, the controller gets the cluster configuration from
its `data` section and starts the creation process of a new cluster (Kafka or Kafka Connect). It's done creating a set
of Kubernetes/OpenShift resources needed for such a cluster, like StatefulSets, ConfigMaps, Services and so on.
...
This might cause rolling update which might lead to service disruption.
...
inconsistent state comparing to the ConfigMaps and what is actually deployed in the Kubernetes/OpenShift cluster.

In order to handle failovers in a proper way, a periodic reconciliation process is executed by the cluster controller so
that it can compare how the ConfigMaps look like (and if they exist) with the current cluster deployment in order to have
...
`kafka-storage` and `zookeeper-storage` fields. It's important to mention that such a storage configuration can't be changed
...
.Kafka Connect cluster ConfigMap
[source,yaml,options="nowrap"]
----
apiVersion: v1
kind: ConfigMap
