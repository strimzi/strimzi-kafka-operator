...
In order to add more connectors plugins to the Kafka Connect deployment, it's possible to use the Stream2Image feature if
thr cluster is running on OpenShift. The cluster configuration is mostly the same as the above example but the following
different type needs to be used.
