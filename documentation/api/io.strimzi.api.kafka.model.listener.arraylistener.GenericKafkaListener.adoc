...
cluster-ip:: Configures an external listener to expose Kafka `cluserIP` type `Services`.
+
This listener type can be helpful if you:
...
* can't enable ssl passthrough in your ingress controller
+

+
Using only one LoadBalancer, brokers could be exposed using ingress controller.
+
...
+
The client connects on the `advertisedPort`, 9190 for broker 0, on `advertisedHost`, ingress.mycompany.com in the following example, but traffic is then routed to the port you configure,
which is `9096` in the following example.
+
`advertisedHost` ingress.mycompany.com should be a dns name routed to ingress LoadBalancer service. In the Ingress Controller LoadBalancer service,
we should add broker exposed ports + extra bootstrap port.
    ...
    listeners:
      - name: external-cluster-ip
        ...
        configuration:
          brokers:
            - broker: 0
              advertisedHost: ingress.mycompany.com
              advertisedPort: 9190
            - broker: 1
              advertisedHost: ingress.mycompany.com
              advertisedPort: 9191
...
----
In your ingress controller configuration you should set properly port->service mapping.

Each broker (clusterIP service) should have different port number to give to ingress controller a selector to choose `Service` to forward to.

bootstrap service should have also a different port than brokers.

.Example `cluster-ip` ingress tcp configmap
[source,yaml,subs="+attributes"]
----
#...
apiVersion: v1
...
metadata:
  name: tcp-services
data:
  "9189": kafka/strimzi-kafka-external-bootstrap:9096
  "9190": kafka/strimzi-kafka-external-0:9096
  "9191": kafka/strimzi-kafka-external-1:9096
  "9192": kafka/strimzi-kafka-external-2:9096
    #...
----

Don't miss to add brokers and bootstrap exposed ports to your ingress LoadBalancer Service.

.Example `cluster-ip` ingress tcp LoadBalancer service
[source,yaml,subs="+attributes"]
----
#...
apiVersion: v1
...
spec:
  ports:
    - name: strimzi-kafka-external-bootstrap
      port: 9189
      protocol: TCP
    - name: strimzi-kafka-external-0
      port: 9190
      protocol: TCP
    - name: strimzi-kafka-external-1
      port: 9191
      protocol: TCP
    - name: strimzi-kafka-external-2
      port: 9192
      protocol: TCP
  type: LoadBalancer
...
* `cluster-ip` listeners use advertised port numbers
