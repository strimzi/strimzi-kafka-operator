...
When rack awareness based on a topology label is enabled, Strimzi adds a node affinity rule to ensure Kafka nodes are scheduled on Kubernetes workers that include the specified `topologyKey` label.
...
Environment-variable-based rack awareness (`type: environment-variable`) is configured through a `envVarName`.
`envVarName` identifies the environment variable that contains the name of the topology in its value.
Users are expected to configure this environment variable on their own, for example using the `template` configuration on a per-node pool basis, or using mutating webhooks and other tools.
...
When rack awareness based on an environment variable is enabled, Strimzi will not add any affinity rules.
Users are expected to configure their own  `affinity` or `topologySpreadConstraints` rules to make sure the nodes are properly distributed across as many racks as possible.
For more information see link:{BookURLDeploying}#assembly-scheduling-str[Configuring pod scheduling^].
