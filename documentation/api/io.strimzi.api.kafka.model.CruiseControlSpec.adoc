...
To access the Cruise Control REST API with security is enabled, you can create API user.
...
# ...
----
1. The name of the API user.
2. Valid API roles are VIEWER, USER, and ADMIN".
For more information on valid API user roles see https://github.com/linkedin/cruise-control/wiki/Security#authorization


When an API user is created, Strimzi generates a random password for that user.
Strimzi creates a new secret with a name of the format <cluster-name>-cruise-control-api-<api-username>.
...
Example secret with API user credentials
[source,yaml,subs="attributes+"]
----
apiVersion: v1
kind: Secret
metadata:
  name: my-cluster-cruise-control-api-my-user
  labels:
...
You can use your own password instead of the one generated by Strimzi.
To do so, create a secret with the password and reference it in the `.spec.cruiseControl.apiUsers[].password` section of the `Kafka` resource.

You can create a Secret setting your own password:
----
kubectl create secret generic cruise-control-api-user-secret-name --from-literal=password=<cruise-control-api-user-password>
----

.Example Secret used for HTTP BASIC authentication for Cruise Control API
----
apiVersion: v1
kind: Secret
metadata:
  name: cruise-control-api-user-secret (1)
type: Opaque
data:
  password: LFTIyFRFlMmU2N2Tm (2)
----
1. The name of the Secret
2. The password stored inside the Secret, base64 encoded.

.Cruise Control configuration to add REST API users with custom password
[source,yaml,subs="attributes+"]
----
apiVersion: {KafkaApiVersion}
kind: Kafka
    ...
    apiUsers:
      - name: my-user (1)
        password:
          valueFrom:
            secretKeyRef:
              name: cruise-control-api-user-secret (2)
              key: password (3)
        role: USER (3)
# ...
----
1. The name of the API user.
2. The name of the secret containing the predefined password.
3. The key for the password stored inside the secret.
4. Valid API roles are VIEWER, USER, and ADMIN".
For more information on valid API user roles see https://github.com/linkedin/cruise-control/wiki/Security#authorization
