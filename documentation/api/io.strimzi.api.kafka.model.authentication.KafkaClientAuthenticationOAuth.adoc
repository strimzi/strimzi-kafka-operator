...
.Username and password
This way of authenticating clients uses the OAuth `Resource Owner Password Grant` mechanism which has been deprecated, and is only supported to enable integration in environments where the `Client Credentials` mechanism cannot be used due to the existing IAM solutions not supporting it or for some reason requiring the use of user accounts for the authentication.

Typical use for this is to create a special user account in your IAM solution (the authorization server) that represents your client application. Give it a long randomly generated password and a very limited set of permissions - for example, it is only allowed to connect to your Kafka cluster but not allowed to use any other services or do any kind of UI login.
...
Normally, you also have to configure a `clientId` using a public OAuth client. If using a confidential OAuth client, you also have to configure a `clientSecret`.
