...
* Other files, which are downloaded and used directly
...
JAR artifacts represent a JAR file which is downloaded and added to a container image.
...
`other` artifacts represent any kind of file which is downloaded and added to a container image.
A file name which will be used in the resulting container image can be also specified using the `fileName` field.
...
<2> (Required) URL from which the artifact is downloaded.
<3> (Optional) SHA-512 checksum to verify the artifact.
<4> (Optional) The name under which the file will be stored in the resulting container image.
