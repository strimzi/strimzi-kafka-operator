...
# 
# The operator-sdk still needs RFEs to be resolved to work without scripts
#
...
generate_bundle_with_operator_sdk() {
  ...
  # The package name must match the name of the bundle
  rm -rf ${MANIFESTS}
  ...
  # The following steps are not preformed by the operator-sdk and must be done manually:
...
generate_bundle_without_operator_sdk() {
  ...
  # Update deployment
  yq ea -i 'select(fi==0).spec.install.spec.deployments[0].spec = select(fi==1).spec | select(fi==0)' ${CSV_FILE} ${CSV_TEMPLATE}
...
generate_image_digests() {
  ...
  do
    ...
    digest=$(curl --silent --header "Accept: application/vnd.docker.distribution.manifest.v2+json" https://$${registry}/v2/$${org}/$${repo}/manifests/$${tag} | sha256sum | head -c 64);
