...
KEYCLOAK_VERSION=$2
WITH_CRD_DELETION_ONLY=$3

SCRIPT_PATH=$(dirname "${BASH_SOURCE[0]}")

if [ $WITH_CRD_DELETION_ONLY == true ];
then
     echo "[INFO] Deleting also Keycloak CRDs"
    ...
    echo "[INFO] $(date -u +"%Y-%m-%d %H:%M:%S") Delete Keycloak & Keycloak Operator (without CRDs)"
