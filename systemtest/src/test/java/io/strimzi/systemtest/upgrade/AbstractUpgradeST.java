...
public class AbstractUpgradeST extends AbstractST {
    ...
    protected void makeSnapshots() {
        coPods = DeploymentUtils.depSnapshot(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, ResourceManager.getCoDeploymentName());
        zkPods = PodUtils.podSnapshot(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, zkSelector);
        kafkaPods = PodUtils.podSnapshot(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, kafkaSelector);
        eoPods = DeploymentUtils.depSnapshot(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, KafkaResources.entityOperatorDeploymentName(clusterName));
        connectPods = PodUtils.podSnapshot(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, connectLabelSelector);
    }
    ...
    protected void changeKafkaAndLogFormatVersion(CommonVersionModificationData versionModificationData, ExtensionContext extensionContext) throws IOException {
        ...
        if (versionModificationData.getProcedures() != null && (!currentLogMessageFormat.isEmpty() || !currentInterBrokerProtocol.isEmpty())) {
            if (kafkaVersionFromProcedure != null && !kafkaVersionFromProcedure.isEmpty() && !kafkaVersionFromCR.contains(kafkaVersionFromProcedure) && extensionContext.getTestClass().get().getSimpleName().toLowerCase(Locale.ROOT).contains("upgrade")) {
                ...
                kafkaPods = RollingUpdateUtils.waitTillComponentHasRolled(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, kafkaSelector, 3, kafkaPods);
            }
            ...
            if (kafkaVersionFromProcedure != null && !kafkaVersionFromProcedure.isEmpty() && !kafkaVersionFromCR.contains(kafkaVersionFromProcedure) && extensionContext.getTestClass().get().getSimpleName().toLowerCase(Locale.ROOT).contains("downgrade")) {
                ...
                cmdKubeClient().patchResource(getResourceApiVersion(Kafka.RESOURCE_PLURAL, operatorVersion), clusterName, "/spec/kafka/version", kafkaVersionFromProcedure);
                LOGGER.info("Waiting for Kafka rolling update to finish");
                kafkaPods = RollingUpdateUtils.waitTillComponentHasRolled(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, kafkaSelector, kafkaPods);
            }
        }
    }
    ...
    protected void waitForKafkaClusterRollingUpdate() {
        ...
        zkPods = RollingUpdateUtils.waitTillComponentHasRolledAndPodsReady(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, zkSelector, 3, zkPods);
        LOGGER.info("Waiting for Kafka StrimziPodSet roll");
        kafkaPods = RollingUpdateUtils.waitTillComponentHasRolledAndPodsReady(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, kafkaSelector, 3, kafkaPods);
        LOGGER.info("Waiting for EO Deployment roll");
        // Check the TO and UO also got upgraded
        eoPods = DeploymentUtils.waitTillDepHasRolled(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, KafkaResources.entityOperatorDeploymentName(clusterName), 1, eoPods);
    }

    protected void waitForReadinessOfKafkaCluster() {
        LOGGER.info("Waiting for ZooKeeper StrimziPodSet");
        RollingUpdateUtils.waitForComponentAndPodsReady(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, zkSelector, 3);
        LOGGER.info("Waiting for Kafka StrimziPodSet");
        RollingUpdateUtils.waitForComponentAndPodsReady(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, kafkaSelector, 3);
        LOGGER.info("Waiting for EO Deployment");
        DeploymentUtils.waitForDeploymentAndPodsReady(io.strimzi.systemtest.Constants.TEST_SUITE_NAMESPACE, KafkaResources.entityOperatorDeploymentName(clusterName), 1);
    }
