...
public class KafkaUpgradeDowngradeST extends AbstractUpgradeST {
    ...
    void testKafkaClusterUpgrade(ExtensionContext testContext) {
        ...
        // ##############################
        ClientUtils.waitForClientsSuccess(producerName, consumerName, Constants.TEST_SUITE_NAMESPACE, continuousClientsMessageCount);
        // ##############################
    }
    ...
    void runVersionChange(TestKafkaVersion initialVersion, TestKafkaVersion newVersion, String producerName, String consumerName, String initLogMsgFormat, String initInterBrokerProtocol, int kafkaReplicas, int zkReplicas, ExtensionContext testContext) {
        ...
        if (KafkaResource.kafkaClient().inNamespace(Constants.TEST_SUITE_NAMESPACE).withName(clusterName).get() == null) {
            ...
            resourceManager.createResourceWithWait(testContext, KafkaTopicTemplates.topic(clusterName, continuousTopicName, 3, 3, 2, Constants.TEST_SUITE_NAMESPACE).build());
            ...
            kafkaPods = PodUtils.podSnapshot(Constants.TEST_SUITE_NAMESPACE, kafkaSelector);
            ...
            if (!sameMinorVersion
                ...
                RollingUpdateUtils.waitTillComponentHasRolled(Constants.TEST_SUITE_NAMESPACE, kafkaSelector, kafkaReplicas, kafkaPods);
            }
        }
        ...
        zkPods = RollingUpdateUtils.waitTillComponentHasRolledAndPodsReady(Constants.TEST_SUITE_NAMESPACE, zkSelector, zkReplicas, zkPods);
        LOGGER.info("1st ZooKeeper roll (image change) is complete");

        // Wait for the kafka broker version change roll
        kafkaPods = RollingUpdateUtils.waitTillComponentHasRolled(Constants.TEST_SUITE_NAMESPACE, kafkaSelector, kafkaPods);
        LOGGER.info("1st Kafka roll (image change) is complete");

        Object currentLogMessageFormat = KafkaResource.kafkaClient().inNamespace(Constants.TEST_SUITE_NAMESPACE).withName(clusterName).get().getSpec().getKafka().getConfig().get("log.message.format.version");
        Object currentInterBrokerProtocol = KafkaResource.kafkaClient().inNamespace(Constants.TEST_SUITE_NAMESPACE).withName(clusterName).get().getSpec().getKafka().getConfig().get("inter.broker.protocol.version");
        ...
        if (isUpgrade && !sameMinorVersion) {
            ...
            if (currentLogMessageFormat == null && TestKafkaVersion.compareDottedVersions(newVersion.protocolVersion(), "3.0") < 0) {
                kafkaPods = RollingUpdateUtils.waitTillComponentHasRolledAndPodsReady(Constants.TEST_SUITE_NAMESPACE, kafkaSelector, kafkaReplicas, kafkaPods);
        ...
        PodUtils.verifyThatRunningPodsAreStable(Constants.TEST_SUITE_NAMESPACE, KafkaResources.kafkaStatefulSetName(clusterName));
        ...
        if (isUpgrade && !sameMinorVersion) {
            ...
            if (currentLogMessageFormat != null || currentInterBrokerProtocol != null) {
                ...
                RollingUpdateUtils.waitTillComponentHasRolledAndPodsReady(Constants.TEST_SUITE_NAMESPACE, kafkaSelector, kafkaReplicas, kafkaPods);
                ...
                PodUtils.verifyThatRunningPodsAreStable(Constants.TEST_SUITE_NAMESPACE, KafkaResources.kafkaStatefulSetName(clusterName));
                assertFalse(RollingUpdateUtils.componentHasRolled(Constants.TEST_SUITE_NAMESPACE, kafkaSelector, kafkaPods));
            }
        }

        if (!isUpgrade) {
            LOGGER.info("Verifying that log.message.format attribute updated correctly to version {}", initLogMsgFormat);
            assertThat(Crds.kafkaOperation(kubeClient().getClient()).inNamespace(Constants.TEST_SUITE_NAMESPACE).withName(clusterName)
