...
public class OlmUpgradeIsolatedST extends AbstractUpgradeST {
    ...
    void testStrimziUpgrade(ExtensionContext extensionContext) throws IOException {
        ...
        UpgradeDowngradeData latestUpgradeData = new UpgradeDowngradeDatalist().getLatestUpgradeData();
        TestKafkaVersion testKafkaVersion = TestKafkaVersion.getLatestSupportedKafkaVersion();
        ...
        latestUpgradeData.setProcedures(new UpgradeKafkaVersion(testKafkaVersion.version()));

        // Set OLM operator version which we want to upgrade to
        latestUpgradeData.setToVersion(Environment.OLM_OPERATOR_LATEST_RELEASE_VERSION);

        // Perform upgrade and verify results
    ...
    private void performUpgradeVerification(UpgradeDowngradeData upgradeData, ExtensionContext extensionContext) throws IOException {
        ...
        clusterOperator.runManualOlmInstallation(upgradeData.getFromOlmChannelName());

        // In this test we intend to setup Kafka once at the beginning and then upgrade it with CO
        File dir = FileUtils.downloadAndUnzip(upgradeData.getUrlFrom());
        File kafkaYaml = new File(dir, upgradeData.getFromExamples() + "/examples/kafka/kafka-persistent.yaml");
        ...
        KafkaTopic kafkaUpgradeTopic = new YAMLMapper().readValue(new File(dir, upgradeData.getFromExamples() + "/examples/topic/kafka-topic.yaml"), KafkaTopic.class);
        ...
        clusterOperator.getOlmResource().upgradeClusterOperator(clusterOperator.getDeploymentNamespace(), upgradeOlmConfig.getCsvName());
