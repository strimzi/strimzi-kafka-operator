...
public class OauthTlsST extends OauthAbstractST {
    ...
    void testProducerConsumerConnect(ExtensionContext extensionContext) {
        ...
        KafkaConnect connect = KafkaConnectTemplates.kafkaConnectWithFilePlugin(clusterName, Constants.TEST_SUITE_NAMESPACE, oauthClusterName, 1)
            .editSpec()
        ...
        resourceManager.createResourceWithWait(extensionContext, connect, ScraperTemplates.scraperPod(Constants.TEST_SUITE_NAMESPACE, scraperName).build());
    ...
    void testMirrorMaker(ExtensionContext extensionContext) {
        ...
        String mirrorMakerPodName = kubeClient().listPodsByPrefixInName(Constants.TEST_SUITE_NAMESPACE, KafkaMirrorMakerResources.deploymentName(oauthClusterName)).get(0).getMetadata().getName();
        String kafkaMirrorMakerLogs = kubeClient().logsInSpecificNamespace(Constants.TEST_SUITE_NAMESPACE, mirrorMakerPodName);
        ...
        resourceManager.createResourceWithWait(extensionContext, KafkaUserTemplates.tlsUser(Constants.TEST_SUITE_NAMESPACE, oauthClusterName, USER_NAME).build());
        KafkaUserUtils.waitForKafkaUserCreation(Constants.TEST_SUITE_NAMESPACE, USER_NAME);
    ...
    void setUp(ExtensionContext extensionContext) {
        ...
        resourceManager.createResourceWithWait(extensionContext, KafkaUserTemplates.tlsUser(Constants.TEST_SUITE_NAMESPACE, oauthClusterName, OAUTH_CLIENT_NAME).build());
    }
}
