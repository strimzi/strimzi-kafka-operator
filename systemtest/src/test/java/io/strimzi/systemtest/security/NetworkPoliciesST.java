...
public class NetworkPoliciesST extends AbstractST {
    ...
    private static final String NETWORK_POLICIES_RBAC_RESOURCE = "networkpolicies";
    private static final String CO_NAMESPACED_ROLE_NAME = "strimzi-cluster-operator-namespaced";
    ...
    void testNPGenerationEnvironmentVariable() {
        ...
        List<String> rbacResources = resourceNames(removeNetworkPolicyRbac());

        LOGGER.info("Cluster Operator (Cluster)Role resources after the removal: {}", rbacResources);

        assertThat("Cluster Operator still has the RBAC permission for NetworkPolicies.",
            rbacResources, not(hasItem(NETWORK_POLICIES_RBAC_RESOURCE)));
    ...
    // Drops networkpolicies from the Cluster Operator (Cluster)Role and returns the rules the API server stored
    private List<PolicyRule> removeNetworkPolicyRbac() {
        if (Environment.isNamespaceRbacScope()) {
            Role role = KubeResourceManager.get().kubeClient().getClient().rbac().roles().inNamespace(Environment.TEST_SUITE_NAMESPACE).withName(CO_NAMESPACED_ROLE_NAME).get();
            List<PolicyRule> originalRules = List.copyOf(role.getRules());

            KubeResourceManager.get().updateResource(new RoleBuilder(role).withRules(rulesWithoutNetworkPolicies(originalRules)).build());

            return KubeResourceManager.get().kubeClient().getClient().rbac().roles().inNamespace(Environment.TEST_SUITE_NAMESPACE).withName(CO_NAMESPACED_ROLE_NAME).get().getRules();
        } else {
            ClusterRole clusterRole = KubeResourceManager.get().kubeClient().getClient().rbac().clusterRoles().withName(CO_NAMESPACED_ROLE_NAME).get();
            List<PolicyRule> originalRules = List.copyOf(clusterRole.getRules());

            KubeResourceManager.get().updateResource(new ClusterRoleBuilder(clusterRole).withRules(rulesWithoutNetworkPolicies(originalRules)).build());

            return KubeResourceManager.get().kubeClient().getClient().rbac().clusterRoles().withName(CO_NAMESPACED_ROLE_NAME).get().getRules();
        }
    }

    private List<PolicyRule> rulesWithoutNetworkPolicies(List<PolicyRule> rules) {
        return rules.stream()
            .map(rule -> new PolicyRuleBuilder(rule)
                .removeFromResources(NETWORK_POLICIES_RBAC_RESOURCE)
                .build())
            .toList();
    }

    private List<String> resourceNames(List<PolicyRule> rules) {
        return rules.stream()
