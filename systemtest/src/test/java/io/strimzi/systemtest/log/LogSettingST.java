...
class LogSettingST extends AbstractST {
    ...
    void testKafkaLogSetting(ExtensionContext extensionContext) {
        ...
        Map<String, String> zkPods = PodUtils.podSnapshot(Environment.TEST_SUITE_NAMESPACE, testStorage.getZookeeperSelector());
        ...
        checkGcLoggingPods(Environment.TEST_SUITE_NAMESPACE, testStorage.getKafkaSelector(), true);
        if (!Environment.isKRaftModeEnabled()) {
            checkGcLoggingPods(Environment.TEST_SUITE_NAMESPACE, testStorage.getZookeeperSelector(), true);
        ...
        if (!Environment.isKRaftModeEnabled()) {
            RollingUpdateUtils.waitTillComponentHasRolledAndPodsReady(Environment.TEST_SUITE_NAMESPACE, testStorage.getZookeeperSelector(), 1, zkPods);
        }
        RollingUpdateUtils.waitTillComponentHasRolledAndPodsReady(Environment.TEST_SUITE_NAMESPACE, testStorage.getKafkaSelector(), 3, kafkaPods);
        ...
        checkGcLoggingPods(Environment.TEST_SUITE_NAMESPACE, testStorage.getKafkaSelector(), false);
        if (!Environment.isKRaftModeEnabled()) {
            checkGcLoggingPods(Environment.TEST_SUITE_NAMESPACE, testStorage.getZookeeperSelector(), false);
    ...
    void testConnectLogSetting(ExtensionContext extensionContext) {
        ...
        resourceManager.createResourceWithWait(extensionContext, KafkaConnectTemplates.kafkaConnect(testStorage.getClusterName(), Environment.TEST_SUITE_NAMESPACE, LOG_SETTING_CLUSTER_NAME, 1)
            .editMetadata()
    ...
    void testMirrorMakerLogSetting(ExtensionContext extensionContext) {
        ...
        resourceManager.createResourceWithWait(extensionContext, KafkaMirrorMakerTemplates.kafkaMirrorMaker(testStorage.getClusterName(), LOG_SETTING_CLUSTER_NAME, GC_LOGGING_SET_NAME, "my-group", 1, false)
            .editMetadata()
                .withNamespace(Environment.TEST_SUITE_NAMESPACE)
            .endMetadata()
            .editSpec()
    ...
    void testBridgeLogSetting(ExtensionContext extensionContext) {
        ...
        resourceManager.createResourceWithWait(extensionContext, KafkaBridgeTemplates.kafkaBridge(testStorage.getClusterName(), LOG_SETTING_CLUSTER_NAME, KafkaResources.plainBootstrapAddress(LOG_SETTING_CLUSTER_NAME), 1)
            .editMetadata()
                .withNamespace(Environment.TEST_SUITE_NAMESPACE)
            .endMetadata()
            .editSpec()
