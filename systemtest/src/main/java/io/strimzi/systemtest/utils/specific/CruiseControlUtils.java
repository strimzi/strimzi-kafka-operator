...
public class CruiseControlUtils {
    ...
    public static Properties getKafkaCruiseControlMetricsReporterConfiguration(String namespaceName, String clusterName) throws IOException {
        String cmName = kubeClient().listPods(namespaceName, LabelSelectors.kafkaLabelSelector(clusterName, KafkaResourceNames.getBrokerComponentName(clusterName))).get(0).getMetadata().getName();
