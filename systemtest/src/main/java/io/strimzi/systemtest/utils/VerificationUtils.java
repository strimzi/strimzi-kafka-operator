...
public class VerificationUtils {
    ...
    public static void assertPodResourceRequests(String namespaceName, String podName, String containerName, String memoryLimit, String cpuLimit, String memoryRequest, String cpuRequest) {
        Pod po = KubeResourceManager.get().kubeClient().getClient().pods().inNamespace(namespaceName).withName(podName).get();
    ...
    public static void verifyContainerEnvVariables(String namespaceName, String podNamePrefix, String containerName, Map<String, String> config) {
        ...
        List<Pod> pods = KubeResourceManager.get().kubeClient().listPodsByPrefixInName(namespaceName, podNamePrefix);
    ...
    public static void verifyConfigMapsLabels(String namespaceName, String clusterName, String additionalClusterName) {
        ...
        KubeResourceManager.get().kubeClient().getClient().configMaps().inNamespace(namespaceName).list().getItems()
            .forEach(cm -> {
    ...
    public static void verifyServiceLabels(String namespaceName, String serviceName, LabelSelector expectedLabels) {
        ...
        Service service = KubeResourceManager.get().kubeClient().getClient().services().inNamespace(namespaceName).withName(serviceName).get();
    ...
    public static void verifyClusterOperatorKafkaDockerImages(String clusterOperatorNamespaceName, String kafkaNamespaceName, String clusterName, int controllerPods, boolean rackAwareEnabled) {
        ...
        List<String> brokerPods = PodUtils.listPodNames(clusterOperatorNamespaceName, LabelSelectors.kafkaLabelSelector(clusterName, KafkaComponents.getBrokerPodSetName(clusterName)));
    ...
    public static void verifyClusterOperatorConnectDockerImage(String clusterOperatorNamespace, String connectNamespaceName, String clusterName) {
        ...
        String connectImageName = PodUtils.getFirstContainerImageNameFromPod(connectNamespaceName, KubeResourceManager.get().kubeClient().listPodsByPrefixInName(connectNamespaceName, KafkaConnectResources.componentName(clusterName)).
