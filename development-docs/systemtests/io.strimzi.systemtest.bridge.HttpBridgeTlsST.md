...
| 2. | Configure HTTP Bridge client for consumption. | HTTP Bridge client is configured with topic and consumer names. |
| 3. | Create Kafka topic with provided configurations. | Kafka topic resource is created and available. |
| 4. | Deploy the HTTP Bridge consumer. | HTTP Bridge consumer starts successfully and is ready to consume messages. |
...
| 7. | Verify message consumption. | Messages are successfully consumed by the HTTP Bridge consumer. |
...
| 3. | Create HTTP Bridge Client job for producing messages. | HTTP Bridge Client job is created and produces messages successfully. |
