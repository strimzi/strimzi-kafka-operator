...
public class ConvertResourceCommand extends AbstractConversionCommand {
    ...
    //protected <T extends HasMetadata, L extends DefaultKubernetesResourceList<T>> List<T> get(String kind, String namespace, boolean allNamespaces) {
    //    MixedOperation<T, L, Resource<T>> op = VERSIONED_OPERATIONS.get(kind).apply(client, TO_API_VERSION);
    //
    //    if (allNamespaces)   {
    //        return op.inAnyNamespace().list().getItems();
    //    } else {
    ...
    // */
    //@SuppressWarnings({"unchecked"})
    //protected <T extends HasMetadata, L extends DefaultKubernetesResourceList<T>> T replace(String kind, T cr) {
    //    MixedOperation<T, L, Resource<T>> op = VERSIONED_OPERATIONS.get(kind).apply(client, TO_API_VERSION);
    //    return op.inNamespace(cr.getMetadata().getNamespace()).resource(cr).update();
    //}
    //
    ...
    // */
    //@SuppressWarnings({"unchecked"})
    //private <T extends HasMetadata, L extends DefaultKubernetesResourceList<T>> T getNamedResource(String kind, String name, String namespace)    {
    //    MixedOperation<T, L, Resource<T>> op = VERSIONED_OPERATIONS.get(kind).apply(client, TO_API_VERSION);
    //
    //    return op.inNamespace(namespace).withName(name).get();
    //}
    ...
    // * @return                  List of found resources which should be converted
    // */
    //private <T extends HasMetadata> List<T> getResources(String[] kinds, String namespace, boolean allNamespaces)    {
    //    List<T> crs = new ArrayList<>();
    //
    //    for (String kind : kinds)   {
    //        crs.addAll(get(kind, namespace, allNamespaces));
    //    }
    ...
    // */
    //@SuppressWarnings({"unchecked"})
    //protected <T extends HasMetadata> T convert(T cr) {
    //    println("Converting " + cr.getKind() + " resource named " + cr.getMetadata().getName() + " from namespace " + cr.getMetadata().getNamespace());
    //    try {
    ...
    // * @throws IOException  Exception if the replacement fails
    // */
    //private <T extends HasMetadata> void convertInKube(T cr) throws IOException {
    //    T convertedCr = convert(cr);
    //    handleMultipartResources(cr.getMetadata().getNamespace());
    //    replace(cr.getKind(), convertedCr);
    //}
    //
    ...
    //        // Get the right resources and convert them
    //        if (name != null)   {
    //             HasMetadata cr = getNamedResource(kinds[0], name, namespace);
    //
    //            if (cr == null) {
    //                throw new IllegalArgumentException("Resource of kind " + kinds[0] + " with name " + name + " in namespace " + namespace + " does not exist!");
    //            }
    ...
    //            convertInKube(cr);
    //        } else {
    //            List<? extends HasMetadata> crs = getResources(kinds, namespace, allNamespaces);
    //
    //            for (HasMetadata cr : crs)   {
    //                convertInKube(cr);
    //            }
    //        }
    //    } catch (IOException e) {
