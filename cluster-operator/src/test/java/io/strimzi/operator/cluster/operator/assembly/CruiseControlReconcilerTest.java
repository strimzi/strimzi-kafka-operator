...
public class CruiseControlReconcilerTest {
    ...
    public void reconcileEnabledCruiseControl(boolean topicOperatorEnabled, VertxTestContext context) {
        ...
        if (topicOperatorEnabled) {
            ...
            doReturn(Map.of(API_TO_ADMIN_NAME_KEY, API_TO_ADMIN_NAME, API_TO_ADMIN_PASSWORD_KEY, "changeit")).when(topicOperatorApiSecret).getData();
            when(mockSecretOps.get(eq(NAMESPACE), eq(KafkaResources.entityTopicOperatorCcApiSecretName(NAME)))).thenReturn(topicOperatorApiSecret);
        }
        ...
        rcnclr.reconcile(false, null, null, Clock.systemUTC())
                .onComplete(context.succeeding(v -> context.verify(() -> {
                    ...
                    if (topicOperatorEnabled) {
                        assertThat(deployCaptor.getAllValues().get(0).getSpec().getTemplate().getMetadata().getAnnotations().get(Annotations.ANNO_STRIMZI_AUTH_HASH), is("dfcbef90"));
                    } else {
