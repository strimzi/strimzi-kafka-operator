...
public class KafkaAssemblyOperatorCustomCertTest {
    ...
    @Test
    public void testWithNoCustomCertificates(VertxTestContext context) throws InterruptedException {
        ...
        kao.createOrUpdate(new Reconciliation("test-trigger", Kafka.RESOURCE_KIND, namespace, clusterName), kafka).setHandler(res -> {
            ...
            StatefulSet reconcileSts = reconcileStsCaptor.getValue();
            assertThat(reconcileSts.getSpec().getTemplate().getMetadata().getAnnotations().get(KafkaCluster.ANNO_STRIMZI_CUSTOM_CERT_THUMBPRINT_TLS_LISTENER), is(nullValue()));
            assertThat(reconcileSts.getSpec().getTemplate().getMetadata().getAnnotations().get(KafkaCluster.ANNO_STRIMZI_CUSTOM_CERT_THUMBPRINT_EXTERNAL_LISTENER), is(nullValue()));
