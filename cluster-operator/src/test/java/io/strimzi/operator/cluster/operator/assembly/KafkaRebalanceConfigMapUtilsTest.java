...
import static io.strimzi.operator.cluster.operator.assembly.KafkaRebalanceProgressUtils.BYTE_MOVEMENT_COMPLETED;
import static io.strimzi.operator.cluster.operator.assembly.KafkaRebalanceProgressUtils.COMPLETED_BYTE_MOVEMENT_KEY;
import static io.strimzi.operator.cluster.operator.assembly.KafkaRebalanceProgressUtils.ESTIMATED_TIME_TO_COMPLETION_KEY;
import static io.strimzi.operator.cluster.operator.assembly.KafkaRebalanceProgressUtils.EXECUTOR_STATE_KEY;
import static io.strimzi.operator.cluster.operator.assembly.KafkaRebalanceProgressUtils.TIME_COMPLETED;
...
public class KafkaRebalanceConfigMapUtilsTest {
    ...
    private void assertFailureWhenMissingField(ObjectNode executorState, String fieldToRemove) {
        ObjectNode modifiedExecutorState = executorState.deepCopy();
        modifiedExecutorState.remove(fieldToRemove);
        assertThrows(RuntimeException.class, () -> updateRebalanceConfigMapWithProgressFields(KafkaRebalanceState.Rebalancing, modifiedExecutorState, null));
    }
    ...
    public void testProgressFieldsForProposalRebalancingState() throws Exception {
        ...
        ObjectNode es1 = createExecutorState(Map.of("finishedDataMovement",  "0",
                "totalDataToMove", "1000",
                "triggeredTaskReason", triggeredTaskTime(1)));
        assertThrows(RuntimeException.class, () -> updateRebalanceConfigMapWithProgressFields(KafkaRebalanceState.Rebalancing, es1, null));

        m =  cm.getData();
        assertFailureWhenMissingField(es1, "finishedDataMovement");
        assertFailureWhenMissingField(es1, "totalDataToMove");
        assertFailureWhenMissingField(es1, "triggeredTaskReason");
        assertThat(m.get(BROKER_LOAD_KEY),  is(BROKER_LOAD));
    }
