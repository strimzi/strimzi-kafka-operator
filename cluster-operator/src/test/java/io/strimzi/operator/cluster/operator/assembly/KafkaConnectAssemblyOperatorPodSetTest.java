...
public class KafkaConnectAssemblyOperatorPodSetTest {
    ...
    @Test
    public void testUpdateLoggingWithoutConnectorsWithLog4j1(VertxTestContext context)  {
        KafkaConnect connect = new KafkaConnectBuilder(CONNECT)
                .editSpec()
                    .withVersion("3.9.0")
    ...
    public void testUpdateLoggingWithoutConnectorsWithLog4j2(VertxTestContext context)  {
        ...
        ops.reconcile(new Reconciliation("test-trigger", KafkaConnect.RESOURCE_KIND, NAMESPACE, NAME))
                .onComplete(context.succeeding(v -> context.verify(() -> {
                    ...
                    PodSetUtils.podSetToPods(podSet).forEach(pod -> {
                        // We use Log4j2 -> this annotation should not be set
                        assertThat(pod.getMetadata().getAnnotations().get(Annotations.ANNO_STRIMZI_LOGGING_HASH), is(nullValue()));
                    });
    ...
    @Test
    public void testUpdateLoggingWithConnectorsWithLog4j1(VertxTestContext context)  {
        KafkaConnect connect = new KafkaConnectBuilder(CONNECT)
                .editMetadata()
                    .addToAnnotations(Annotations.STRIMZI_IO_USE_CONNECTOR_RESOURCES, "true")
                .endMetadata()
    ...
    public void testUpdateLoggingWithConnectorsWithLog4j2(VertxTestContext context)  {
        ...
        ops.reconcile(new Reconciliation("test-trigger", KafkaConnect.RESOURCE_KIND, NAMESPACE, NAME))
                .onComplete(context.succeeding(v -> context.verify(() -> {
                    ...
                    PodSetUtils.podSetToPods(podSet).forEach(pod -> {
                        // With Log4j2, this annotation should not be set anymore
                        assertThat(pod.getMetadata().getAnnotations().get(Annotations.ANNO_STRIMZI_LOGGING_HASH), is(nullValue()));
                    });
