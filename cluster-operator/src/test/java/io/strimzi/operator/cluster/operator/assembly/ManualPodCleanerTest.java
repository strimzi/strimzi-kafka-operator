...
public class ManualPodCleanerTest {
    ...
    @Test
    public void testManualPodCleanupOnePodWithPodSets() {
        ...
        manualPodCleanup(pods, pvcs, List.of(CONTROLLER_NAME + "-1"), List.of("data-" + CONTROLLER_NAME + "-1"));
    }

    @Test
    public void testManualPodCleanupJbodWithPodSets() {
        List<Pod> pods = List.of(
                podWithName(CONTROLLER_NAME + "-0"),
                podWithNameAndAnnotations(CONTROLLER_NAME + "-1", Collections.singletonMap(Annotations.ANNO_STRIMZI_IO_DELETE_POD_AND_PVC, "true")),
                podWithName(CONTROLLER_NAME + "-2")
        ...
        manualPodCleanup(pods, pvcs, List.of(CONTROLLER_NAME + "-1"), List.of("data-0-" + CONTROLLER_NAME + "-1", "data-1-" + CONTROLLER_NAME + "-1"));
    }

    @Test
    public void testManualPodCleanupMultiplePodsWithPodSets() {
        List<Pod> pods = List.of(
                podWithName(CONTROLLER_NAME + "-0"),
                podWithNameAndAnnotations(CONTROLLER_NAME + "-1", Collections.singletonMap(Annotations.ANNO_STRIMZI_IO_DELETE_POD_AND_PVC, "true")),
    ...
    @Test
    public void testManualPodCleanupNoPodsWithPodSets() {
        List<Pod> pods = List.of(
                podWithName(CONTROLLER_NAME + "-0"),
        ...
        manualPodCleanup(pods, pvcs, List.of(), List.of());
    }

    private void manualPodCleanup(List<Pod> pods, List<PersistentVolumeClaim> pvcs, List<String> podsToBeDeleted, List<String> pvcsToBeDeleted) {
        ...
        when(mockPodSetOps.getAsync(any(), eq(CONTROLLER_NAME))).thenReturn(CompletableFuture.completedFuture(new StrimziPodSetBuilder().withNewMetadata().withName(CONTROLLER_NAME).endMetadata().withNewSpec().withPods(PodSetUtils.podsToMaps(pods)).endSpec().build()));
        when(mockPodSetOps.reconcile(any(), any(), eq(CONTROLLER_NAME), podSetReconciliationCaptor.capture())).thenReturn(CompletableFuture.completedFuture(null));

        PodOperator mockPodOps = supplier.podOperations;
        when(mockPodOps.listAsync(any(), eq(SELECTOR))).thenReturn(CompletableFuture.completedFuture(pods));
        ArgumentCaptor<String> podDeletionCaptor = ArgumentCaptor.forClass(String.class);
        when(mockPodOps.deleteAsync(any(), any(), podDeletionCaptor.capture(), anyBoolean())).thenReturn(CompletableFuture.completedFuture(null));
        when(mockPodOps.readiness(any(), any(), any(), anyLong(), anyLong())).thenReturn(CompletableFuture.completedFuture(null));

        PvcOperator mockPvcOps = supplier.pvcOperations;
        when(mockPvcOps.listAsync(any(), eq(SELECTOR))).thenReturn(CompletableFuture.completedFuture(pvcs));
        ArgumentCaptor<String> pvcDeletionCaptor = ArgumentCaptor.forClass(String.class);
        when(mockPvcOps.deleteAsync(any(), any(), pvcDeletionCaptor.capture(), anyBoolean())).thenReturn(CompletableFuture.completedFuture(null));
        ...
        cleaner.maybeManualPodCleaning().toCompletableFuture().join();

        if (!podsToBeDeleted.isEmpty()) {
            // PodSet was reconciled to remove the pod from it
            assertThat(podSetReconciliationCaptor.getAllValues().size(), is(1));
            assertThat(podSetReconciliationCaptor.getAllValues().getFirst().getSpec().getPods().size(), is(2));
        } else {
