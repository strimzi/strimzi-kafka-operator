...
public class KafkaAssemblyOperatorCustomCertStatefulSetMockTest {
    ...
    public void setup() {
        ...
        client.secrets().inNamespace(namespace).resource(getTlsSecret()).create();
        client.secrets().inNamespace(namespace).resource(getExternalSecret()).create();
    ...
    public void testPodToRestartIsEmptyAndNoCustomCertAnnotationsWhenNoCustomCertificates(VertxTestContext context) {
        kafka = new KafkaBuilder(createKafka())
                ...
                .endSpec()
                .build();
        Crds.kafkaOperation(client).inNamespace(namespace).resource(kafka).replace();
