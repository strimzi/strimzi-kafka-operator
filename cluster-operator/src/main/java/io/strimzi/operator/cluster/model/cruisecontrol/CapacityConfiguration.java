...
public class CapacityConfiguration {
    ...
    private void generateCapacityEntries(CruiseControlSpec spec,
        ...
        for (NodeRef node : kafkaBrokerNodes) {
            ...
            NetworkCapacity inboundNetwork = new NetworkCapacity(INBOUND_NETWORK, generalBrokerCapacity, brokerCapacityOverride);
            NetworkCapacity outboundNetwork = new NetworkCapacity(OUTBOUND_NETWORK, generalBrokerCapacity, brokerCapacityOverride);
     ...
     * <p>
     * Each {@code CapacityEntry} defines resource capacities for a specific broker, including CPU,
     * disk, inbound network and outbound network.
     * </p>
     *
     * @param id               The broker ID.
     * @param cpu              The CPU capacity, typically expressed in number of cores.
     * @param disk             The disk capacity, typically expressed in MiB.
     * @param inboundNetwork   The inbound network capacity, typically expressed in KiB.
     * @param outboundNetwork  The outbound network capacity, typically expressed in KiB.
    ...
    public record CapacityEntry(
            ...
            CpuCapacity cpu,
            DiskCapacity disk,
            NetworkCapacity inboundNetwork,
            NetworkCapacity outboundNetwork,
            String doc
    ) {
        private CapacityEntry(int id, CpuCapacity cpu, DiskCapacity disk, NetworkCapacity inboundNetwork, NetworkCapacity outboundNetwork) {
            this(id, cpu, disk, inboundNetwork, outboundNetwork, "Capacity for Broker " + id);
        }
    }
        ...
        for (CapacityEntry capacityEntry : capacityEntries.values()) {
            JsonObject capacityEntryJson = new JsonObject()
                    ...
                    .put(CAPACITY_KEY, new JsonObject()
                            .put(DISK.getKey(), capacityEntry.disk.getJson())
                            .put(CPU.getKey(), capacityEntry.cpu.getJson())
                            .put(INBOUND_NETWORK.getKey(), capacityEntry.inboundNetwork.toString())
                            .put(OUTBOUND_NETWORK.getKey(), capacityEntry.outboundNetwork.toString())
                    )
                    .put(DOC_KEY, capacityEntry.doc());
            capacityList.add(capacityEntryJson);
        }
        JsonObject config = new JsonObject();
        config.put(CAPACITIES_KEY, capacityList);
