 ...
 * @param issuedTimestamp       Time when the token was issued as milliseconds since the epoch
 * @param expirationTimestamp   Time when the token expires as milliseconds since the epoch
 */
public record ServiceAccountToken(String value, long issuedTimestamp, long expirationTimestamp) {
    /**
     ...
     */
    public boolean isUsableAt(long timestamp, double threshold) {
        return timestamp < issuedTimestamp + (long) (threshold * (expirationTimestamp - issuedTimestamp));
    }
}
