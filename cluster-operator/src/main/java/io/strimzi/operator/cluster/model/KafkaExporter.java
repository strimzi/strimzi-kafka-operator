    ...
    public static KafkaExporter fromCrd(Reconciliation reconciliation, Kafka kafkaAssembly, KafkaVersion.Lookup versions) {
        ...
        if (spec != null) {
            ...
            if (image == null) {
                ...
                image = System.getenv().getOrDefault(ClusterOperatorConfig.KAFKA_EXPORTER_IMAGE, versions.kafkaImage(kafkaClusterSpec.getImage(), versions.defaultVersion().version()));
            }
            result.image = image;
