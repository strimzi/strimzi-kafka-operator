        ...
        if (artifactMap.size() > 0) {
            ...
            artifactMap.forEach((plugin, mvnList) ->
                mvnList.forEach(mvn -> {
                    ...
                    Cmd cmd;
                    String includeScopeParam = mvn.getIncludeScope() != null ? "-DincludeScope=" + mvn.getIncludeScope().toValue() : null;
                    if (Boolean.TRUE.equals(mvn.getInsecure()))    {
                        ...
                        if (includeScopeParam != null) {
                            cmd.andRun("mvn", "dependency:copy-dependencies", "-s", settingsFile,
                        ...
                        cmd.andRun("curl", "-f", "-k", "-L", "--create-dirs", "--output",
                        ...
                        cmd.andRun("curl", "-f", "-L", "--create-dirs", "--output",
                                        "/tmp/artifacts/" + artifactDir + "/" + mvn.getArtifact() + "-" + mvn.getVersion() + ".jar",
                                        assembleResourceUrl(repo, mvn, "jar"));
                    }
