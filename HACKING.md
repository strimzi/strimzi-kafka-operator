...
To set custom Kafka version in system tests need to add system property `ST_KAFKA_VERSION` with one of the following values: `2.2.1`, `2.3.0`. For more info about allowed versions see [kafka-versions](https://github.com/strimzi/strimzi-kafka-operator/blob/master/kafka-versions).
